	...
	function wpqa_edit_profile($atts, $content = null) {
		...
		if (!is_user_logged_in()) {
			...
			$out .= '<form class="edit-profile-form wpqa_form" method="post" enctype="multipart/form-data">'.apply_filters('wpqa_edit_profile_form','edit_profile');
				...
				$your_cover = get_the_author_meta('your_cover',$user_info->ID);
				$rand_e = rand(1,1000);
					...
					<div class="page-sections" id="edit-profile"'.(isset($_POST["profile_type"]) && $_POST["profile_type"] == "setting"?" style='display: block'":(isset($_POST["profile_type"]) && $_POST["profile_type"] == "password"?" style='display: none'":"")).'>';
						if (isset($edit_profile_sections) && is_array($edit_profile_sections)) {
							foreach ($edit_profile_sections as $key_sections => $value_sections) {
											...
											foreach ($edit_profile_items_1 as $key_items_1 => $value_items_1) {
												if ($key_items_1 == "names" && isset($value_items_1["value"]) && $value_items_1["value"] == "names") {
													...
													if ($user_info->ID > 0) {
														...
														<div class="author-image profile-image">
															...
															if (((!is_array($you_avatar) && $you_avatar != "") || (is_array($you_avatar) && isset($you_avatar["id"]) && $you_avatar["id"] != 0))) {
																$out .= '<div class="clearfix"></div>
																<div class="button-default wpqa-remove-image" data-name="you_avatar" data-type="user_meta" data-id="'.$user_info->ID.'" data-nonce="'.wp_create_nonce("wpqa_remove_image").'">'.esc_html__("Delete","wpqa").'</div>
													...
													<div class="fileinputs">
														<input type="file" name="you_avatar" id="you_avatar_'.$rand_e.'">
														<div class="fakefile">
													...
													if ($user_info->ID > 0 && ((!is_array($your_cover) && $your_cover != "") || (is_array($your_cover) && isset($your_cover["id"]) && $your_cover["id"] != 0))) {
														...
														<div class="author-image profile-image">
															<span class="author-image-span wpqa-delete-image-span">'.wpqa_get_user_cover(array("user_id" => $user_info->ID,"size" => 100,"user_name" => $user_info->display_name)).'</span>
																<div class="clearfix"></div>
																<div class="button-default wpqa-remove-image" data-name="your_cover" data-type="user_meta" data-id="'.$user_info->ID.'" data-nonce="'.wp_create_nonce("wpqa_remove_image").'">'.esc_html__("Delete","wpqa").'</div>
																<div class="loader_2 loader_4"></div>
														</div>';
													}
													$out .= '<label for="your_cover_'.$rand_e.'">'.esc_html__('Cover Picture','wpqa').($profile_cover_required == "on"?'<span class="required">*</span>':'').'</label>
													<div class="fileinputs">
														<input type="file" name="your_cover" id="your_cover_'.$rand_e.'">
														<div class="fakefile">
															<button type="button">'.esc_html__('Select file','wpqa').'</button>
															<span>'.esc_html__('Browse','wpqa').'</span>
														</div>
														...
														<span class="styled-select">
															<select name="country" id="country_'.$rand_e.'" '.($country_required == "on"?'class="required-item"':'').'>
																<option value="">'.esc_html__( 'Select a country&hellip;', 'wpqa' ).'</option>';
																	foreach( wpqa_get_countries() as $key => $value ) {
								...
								}else if (isset($value_sections["value"]) && $value_sections["value"] == "social" && isset($edit_profile_items_2) && is_array($edit_profile_items_2)) {
									$out .= '<div class="page-section">
										<div class="page-wrap-content">
											...
											<div class="wpqa_form_2">';
												foreach ($edit_profile_items_2 as $key_items_2 => $value_items_2) {
													...
													}else if ($key_items_2 == "gplus" && isset($value_items_2["value"]) && $value_items_2["value"] == "gplus") {
														$out .= '<p>
															<label for="google_'.$rand_e.'">'.esc_html__("Google plus","wpqa").'  '.esc_html__("(Put the full URL)","wpqa").'</label>
															<input type="text" name="google" id="google_'.$rand_e.'" value="'.(isset($_POST["google"])?esc_url($_POST["google"]):esc_url($google)).'">
...
if (!function_exists('wpqa_process_edit_profile_form')) :
	function wpqa_process_edit_profile_form() {
		...
		$get_your_cover = get_user_meta($user_id,"your_cover",true);
		...
		$posted = array(
			...
			'google'              => (isset($_POST['google']) && $_POST['google'] != ""?esc_url($_POST['google']):""),
		...
		if (empty($_FILES['you_avatar']['name']) && empty($get_you_avatar) && $profile_picture === "on" && $profile_picture_required == "on") {
			$errors->add('required-field','<strong>'.esc_html__("Error","wpqa").' :&nbsp;</strong> '.esc_html__("There are required fields (Profile picture).","wpqa"));
		}
		if (empty($_FILES['your_cover']['name']) && empty($get_your_cover) && $profile_cover === "on" && $profile_cover_required == "on") {
		...
		if (isset($_FILES['you_avatar']) && !empty($_FILES['you_avatar']['name'])) :
			...
			$file_tmp = $_FILES['you_avatar']['tmp_name'];
		...
		if (isset($_FILES['your_cover']) && !empty($_FILES['your_cover']['name'])) :
			$mime = $_FILES["your_cover"]["type"];
			$file_tmp = $_FILES['your_cover']['tmp_name'];
			$size = filesize($file_tmp);
			if (($mime != 'image/jpeg') && ($mime != 'image/jpg') && ($mime != 'image/png')) {
				$errors->add('upload-error', esc_html__('Error type, Please upload: jpg,jpeg,png','wpqa'));
				if ($errors->get_error_code()) return $errors;
				...
				$your_cover = wp_handle_upload($_FILES['your_cover'],array('test_form'=>false),current_time('mysql'));
				if ($your_cover && isset($your_cover["url"])) :
					...
					$attach_data = wp_generate_attachment_metadata( $attach_id, $filename );
					wp_update_attachment_metadata( $attach_id, $attach_data );
					update_user_meta($user_id,"your_cover",$attach_id);
				endif;
			...
			update_user_meta($user_id,"your_cover",$get_your_cover);
		endif;
	...
	if (isset($_POST['from_admin']) && $_POST['from_admin'] == "yes") {
		...
		if (is_super_admin($get_current_user_id) && $active_points == "on") {
			...
			if ($the_points > 0) {
				...
				add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$the_points,$add_remove_point_last,$the_reason,"time" => current_time('timestamp')));
			}
		}
		...
		$post_array = array('country','city','phone','gender','age','facebook','twitter','youtube','vimeo','linkedin','google','instagram','pinterest','follow_email','show_point_favorite','received_email','received_message','profile_credential');
