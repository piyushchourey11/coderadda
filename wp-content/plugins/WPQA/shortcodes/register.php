...
if (!function_exists('wpqa_signup_attr')) :
	function wpqa_signup_attr($atts, $content = null) {
		...
		if (is_user_logged_in()) {
			...
			$rand_w = rand(1,1000);
			if (shortcode_exists('wpqa_social_login') || shortcode_exists('oa_social_login') || shortcode_exists('wordpress_social_login') || shortcode_exists('apsl-login') || shortcode_exists('apsl-login-lite') || shortcode_exists('nextend_social_login')) {
				...
				if ( !$errors->get_error_code() ) :
					...
					if (is_wp_error($user_id)) {
						...
						if ($confirm_email == "on") {
							...
							update_option("Ahmed",$last_message_email);
							$email_title = wpqa_options("title_confirm_link_2");
							$email_title = ($email_title != ""?$email_title:esc_html__("Confirm account","wpqa"));
							wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),esc_html($_POST['email']),$email_title,$last_message_email);
						...
						wp_update_user( array ('ID' => $user_id, 'role' => ($confirm_email == "on"?'activation':$default_group), 'first_name' => $posted['first_name'], 'last_name' => $posted['last_name'], 'display_name' => $posted['display_name'],'user_nicename' => $posted['user_name'],'nickname' => $posted['user_name']) ) ;
...
if (!function_exists('wpqa_registration_save')) :
	function wpqa_registration_save ($user_id) {
		...
		if ($user_id > 0 && $gift_site == "" && $point_new_user > 0 && $active_points == "on") {
			...
			add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$point_new_user,"+","gift_site","","","time" => current_time('timestamp')));
