	...
	function wpqa_pass_jquery() {
		...
		if (wpqa_is_ajax()) :
			...
			if ( !$errors->get_error_code() ) :
				...
				$get_user_by_mail = get_user_by('email',$_POST['user_mail']);
				...
				wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),esc_html($_POST['user_mail']),$email_title,$last_message_email);
		...
		if (isset($_GET['reset_password']) && isset($_GET['u'])) {
			...
			if (!is_user_logged_in()) :
				...
				if ($reset_password == (int)$_GET['reset_password']) :
					...
					$email_title = ($email_title != ""?$email_title:esc_html__("Reset your password","wpqa"));
					wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),$author_user_email,$email_title,$last_message_email);
