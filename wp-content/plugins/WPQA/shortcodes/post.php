		...
		if (is_wp_error($return)) :
   			...
   			if ($get_post->post_type == "post") {
				...
				if ($post_status == "draft") {
					...
					wpqa_session('<div class="alert-message success"><i class="icon-check"></i><p>'.esc_html__("The post has added successfully, It's under review.","wpqa").'</p></div>','wpqa_session');
					if ($user_id > 0) {
...
function wpqa_process_new_posts() {
	...
	if ($form_type == "add_post") {
		...
		if (wpqa_options("content_post") == "on") {
		...
		if (!is_user_logged_in() && $add_post_no_register == "on" && $user_id == 0) {
			...
			if ($point_add_post > 0 && $active_points == "on") {
				$_points = get_user_meta($user_id,$user_id."_points",true);
				$_points++;
			
				update_user_meta($user_id,$user_id."_points",$_points);
				add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$point_add_post,"+","add_post",$post_id,"time" => current_time('timestamp')));
...
function wpqa_add_edit_post($type,$popup = false) {
	...
	$add_post_no_register = wpqa_options("add_post_no_register");
	$tags_post = wpqa_options("tags_post");
	$attachment_post = wpqa_options("attachment_post");
	...
	$out = '<form class="form-post wpqa_form" method="post" enctype="multipart/form-data">'.(isset($_POST["form_type"]) && $_POST["form_type"] == $type."_post"?apply_filters('wpqa_'.$type.'_post',$type):"").'
		<div class="form-inputs clearfix">';
			...
			if ($attachment_post == "on") {
				...
				$out .= '<label for="attachment-'.$rand.'">'.esc_html__("Featured image","wpqa").'</label>
			...
			$out .= '<div class="wpqa_textarea'.($editor_post_details == "on"?"":" wpqa_textarea_p").'">
				<label for="post-details-'.$rand.'">'.esc_html__("Details","wpqa").(wpqa_options("content_post") == "on"?'<span class="required">*</span>':'').'</label>';
			...
			'.apply_filters('wpqa_add_edit_post_after_details',false,$type,$post_add,$post_edit,(isset($get_post)?$get_post:0));
