...
function wpqa_categories_checklist ($args = array()) {
	...
	foreach ($categories as $key => $value) {
		$output .= '<li id="'.$args['name'].$taxonomy.'-'.$value->term_id.'">
			<label class="selectit"><span class="wpqa_checkbox"><input value="'.$value->term_id.'" '.(is_array($args['selected_cats']) && in_array($value->term_id,$args['selected_cats'])?checked($value->term_id,$value->term_id,false):'').' type="checkbox" name="'.$args['name'].'[]" id="'.$args['name'].'in-'.$taxonomy.'-'.$value->term_id.'"></span> '.$value->name.'</label>
		</li>';
	}
	return $output;
}
	   			...
	   			if ($question_status == "draft") {
	   				...
	   				if ($send_email_draft_questions == "on") {
	   					$send_text = wpqa_send_email(wpqa_options("email_draft_questions"),"",$return);
	   						...
	   						wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),get_bloginfo("admin_email"),$email_title,$last_message_email);
	   				}
					wpqa_session('<div class="alert-message success"><i class="icon-check"></i><p>'.esc_html__("The question has added successfully, It's under review.","wpqa").'</p></div>','wpqa_session');
...
function wpqa_process_new_questions() {
	...
	if ($form_type == "add_question") {
		...
		if (!is_user_logged_in() && $ask_question_no_register == "on" && $user_id == 0) {
			...
			$active_points = wpqa_options("active_points");
			if ($point_add_question > 0 && $active_points == "on") {
				$_points = get_user_meta($user_id,$user_id."_points",true);
				$_points++;
			
				update_user_meta($user_id,$user_id."_points",$_points);
				add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$point_add_question,"+","add_question",$question_id,"time" => current_time('timestamp')));
			...
			if ($points && $question_points_active == "on") {
				$_points = get_user_meta($user_id,$user_id."_points",true);
				$_points++;
			
				update_user_meta($user_id,$user_id."_points",$_points);
				add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$question_points,"-","question_point",$question_id,"time" => current_time('timestamp')));
...
function wpqa_question($atts, $content = null) {
	...
	if (($custom_permission == "on" && is_user_logged_in() && !is_super_admin($user_get_current_user_id) && empty($roles["ask_question"])) || ($custom_permission == "on" && !is_user_logged_in() && $ask_question != "on")) {
		...
		if (!is_user_logged_in() && $pay_ask == "on") {
			...
			$_allow_to_ask = (isset($user_get_current_user_id) && $user_get_current_user_id != ""?get_user_meta($user_get_current_user_id,$user_get_current_user_id."_allow_to_ask",true):"");
			if ($user_get_current_user_id > 0 && isset($_POST["process"]) && $_POST["process"] == "ask") {
				...
				if (isset($_POST["points"]) && $_POST["points"] > 0) {
					...
					if ($points_price <= $points_user) {
						$_points = get_user_meta($user_get_current_user_id,$user_get_current_user_id."_points",true);
						$_points++;
					
						update_user_meta($user_get_current_user_id,$user_get_current_user_id."_points",$_points);
						add_user_meta($user_get_current_user_id,$user_get_current_user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$points_price,"-","ask_points","","time" => current_time('timestamp')));

						update_user_meta($user_get_current_user_id,"points",$points_user-$points_price);
						$_SESSION['wpqa_session'] = '<div class="alert-message success"><i class="icon-check"></i><p>'.esc_html__("You have just bought to ask question by points.","wpqa").'</p></div>';
					}else {
						$buy_points_payment = wpqa_options("buy_points_payment");
						$_SESSION['wpqa_session'] = '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you haven't enough points","wpqa").($buy_points_payment == "on"?', <a href="'.wpqa_buy_points_permalink().'">'.esc_html__("You can buy the points from here.","wpqa").'</a>':'.').'</p></div>';
			...
			if (isset($_allow_to_ask) && (int)$_allow_to_ask < 1 && $pay_ask == "on" && $payment_group[$user_login_group] != $user_login_group) {
				...
				if ($question_points_active != "on" || ($points >= $question_points && $question_points_active == "on")) {
...
function wpqa_add_edit_question($type,$popup = false,$user = false) {
	...
	if ($user == "user" && isset($get_user_id) && $get_user_id != "") {
			...
			if (isset($question_sort) && is_array($question_sort)) {
				foreach ($question_sort as $sort_key => $sort_value) {
					if ($sort_key == "categories_question" && $user != "user" && isset($question_sort["categories_question"]["value"]) && $question_sort["categories_question"]["value"] == "categories_question") {
						$category_single_multi = wpqa_options("category_single_multi");
						$out .= '<'.($category_single_multi == 'multi'?'div':'p').' class="wpqa_category">
							<label for="question-category-'.$rand.'">'.esc_html__("Category","wpqa").'<span class="required">*</span></label>
							'.wpqa_select_categories($rand,($type == "add" && isset($question_add['category'])?$question_add['category']:($type == "edit"?(isset($question_edit['category'])?$question_edit['category']:$question_category):"")),null,($type == 'edit'?$get_question:''),'question-category').'
						...
						</'.($category_single_multi == 'multi'?'div':'p').'>';
						...
						if ($type == "add") {
							$out .= '<div class="question-multiple-upload">
						...
						$out .= '<p class="wpqa_checkbox_p">
							<label for="video_description-'.$rand.'">
								...
								<span class="wpqa_checkbox_span">'.esc_html__("Do you need a video to description the problem better?","wpqa").'</span>
							</label>
						</p>
			...
			if (is_user_logged_in() && is_super_admin($user_id) && $user != "user") {
				$out .= '<p class="wpqa_checkbox_p">
					<label for="sticky-'.$rand.'">
						...
						<span class="wpqa_checkbox_span">'.esc_html__("Stick this question","wpqa").'</span>
					</label>
				</p>';
			}
