(function($) { "use strict";
	...
	if (jQuery(".button-hide-click").length) {
					...
					success:function(result) {
						if (result == "best") {
							if (type == "best_answer_a") {
								...
								jQuery("#comment-"+comment_id+" .last-item-answers").before('<li><a class="best_answer_re" data-nonce="'+wpqa_js.wpqa_best_answer_nonce+'" href="#" title="'+wpqa_js.cancel_best_answer+'"><i class="icon-cancel"></i>'+wpqa_js.cancel_best_answer+'</a></li>');
							}else {
							...
							jQuery("#comment-"+result+" .last-item-answers").before('<li><a class="best_answer_re" data-nonce="'+wpqa_js.wpqa_best_answer_nonce+'" href="#" title="'+wpqa_js.cancel_best_answer+'"><i class="icon-cancel"></i>'+wpqa_js.cancel_best_answer+'</a></li>');
	...
	function wpqa_report() {
		if (jQuery(".report_q,.report_c").length) {
			...
			jQuery(".submit-report").submit(function () {
				...
				jQuery('.load_span',report_v).show().css({"display":"block"});
				if (explain.val() == '') {
					explain.css("border-color","#e1e2e3");
					if (explain.val() == '') {
						explain.css("border-color","#F00");
					}
					jQuery(".wpqa_error",report_v).html('<span class="required-error">'+wpqa_js.wpqa_error_text+'</span>').animate({opacity: 'show' , height: 'show'}, 400).delay(1200).animate({opacity: 'hide' , height: 'hide'}, 400);
				}else {
					...
					jQuery.post(wpqa_js.admin_url,fromWithAction,function(data) {
						if (data == "deleted_report") {
							...
							jQuery(".wpqa_success",report_v).text(wpqa_js.reported).animate({opacity: 'show' , height: 'show'}, 400).delay(1200).animate({opacity: 'hide' , height: 'hide'}, 400);
						}
					});
				}
				jQuery('.load_span',report_v).hide().css({"display":"none"});
	...
	jQuery(".poll_results").live("click",function() {
		jQuery(this).closest(".poll-area").find(".poll_2").fadeOut(200);
		jQuery(this).closest(".poll-area").find(".poll_1").delay(500).slideDown(200);
		return false;
	});
	
	jQuery(".poll_polls").live("click",function() {
		jQuery(this).closest(".poll-area").find(".poll_1").fadeOut(200);
		jQuery(this).closest(".poll-area").find(".poll_2").delay(500).slideDown(200);
		return false;
	});
		...
		if (jQuery(whatClass).length) {
			jQuery(whatClass).submit(function() {
				...
				if (jQuery('.wpqa_captcha',thisform).length) {
					...
					if (wpqa_captcha.val() == "") {
						jQuery(".wpqa_error",thisform).html('<span class="required-error required-error-c">'+wpqa_js.wpqa_error_text+'</span>').animate({opacity: 'show' , height: 'show'}, 400).delay(1200).animate({opacity: 'hide' , height: 'hide'}, 400);
						...
						if (wpqa_captcha.val() != wpqa_js.captcha_answer) {
							jQuery(".wpqa_error",thisform).html('<span class="required-error required-error-c">'+wpqa_js.wpqa_error_captcha+'</span>').animate({opacity: 'show' , height: 'show'}, 400).delay(1200).animate({opacity: 'hide' , height: 'hide'}, 400);
							wpqa_captcha.css("border-color","#F00");
							jQuery('.load_span',thisform).hide().css({"display":"none"});
							jQuery('input[type="submit"]',thisform).show();
							return false;
				...
				jQuery.post(wpqa_js.admin_url,fromWithAction,function(response) {
					...
					if (result.success == 1) {
						if (whatAction == "password") {
							...
							jQuery(".wpqa_success",thisform).html(result.done).animate({opacity: 'show' , height: 'show'}, 400).delay(1200).animate({opacity: 'hide' , height: 'hide'}, 400);
						}else {
							window.location = result.redirect;
						}
					}else if (result.error) {
						jQuery(".wpqa_error",thisform).html('<span class="required-error">'+result.error+'</span>').animate({opacity: 'show' , height: 'show'}, 400).delay(1200).animate({opacity: 'hide' , height: 'hide'}, 400);
					}else {
						return true;
					}
					jQuery('.load_span',thisform).hide().css({"display":"none"});
	...
	if (jQuery("#commentform").length) {
		jQuery("#commentform").attr((wpqa_js.attachment_answer == "on" || wpqa_js.featured_image_answer == "on"?"enctype":"data-empty"),(wpqa_js.attachment_answer == "on" || wpqa_js.featured_image_answer == "on"?"multipart/form-data":"none")).submit(function () {
			...
			if (jQuery('.wpqa_captcha',thisform).length) {
				...
				wpqa_captcha.css("border-color","#e1e2e3");
				if (wpqa_captcha.val() == "") {
					wpqa_captcha.css("border-color","#F00").parent().parent().parent().find(".wpqa_error").html('<span class="required-error required-error-c">'+wpqa_js.wpqa_error_captcha+'</span>').animate({opacity: 'show' , height: 'show'}, 400).delay(1200).animate({opacity: 'hide' , height: 'hide'}, 400);
					jQuery("#commentform .load_span").hide();
					jQuery("#commentform .button-hide-click").show();
					return false;
				}else if (wpqa_captcha.hasClass("captcha_answer")) {
