	...
	function wpqa_question_vote_up() {
		...
		if ((is_user_logged_in() && is_array($count_up) && in_array($get_current_user_id,$count_up)) || (!is_user_logged_in() && isset($_COOKIE[wpqa_options("uniqid_cookie").'wpqa_question_vote_up'.$id]) && $_COOKIE[wpqa_options("uniqid_cookie").'wpqa_question_vote_up'.$id] == "wpqa_yes")) {
			...
			if ($user_id != $get_current_user_id && $user_id > 0 && $point_voting_question > 0 && $active_points == "on") {
				...
				add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),($point_voting_question != ""?$point_voting_question:1),"+","voting_question",$id,"time" => current_time('timestamp')));
			
				$points_user = get_user_meta($user_id,"points",true);
				update_user_meta($user_id,"points",$points_user+($point_voting_question != ""?$point_voting_question:1));
			}
	...
	function wpqa_comment_vote_up() {
		...
		if ((is_user_logged_in() && is_array($count_up) && in_array($get_current_user_id,$count_up)) || (!is_user_logged_in() && isset($_COOKIE[wpqa_options("uniqid_cookie").'wpqa_comment_vote_up'.$id]) && $_COOKIE[wpqa_options("uniqid_cookie").'wpqa_comment_vote_up'.$id] == "wpqa_yes_comment")) {
			...
			if ($user_votes_id != $get_current_user_id && $user_votes_id > 0 && $point_voting_answer > 0 && $active_points == "on") {
				...
				$_points = get_user_meta($user_votes_id,$user_votes_id."_points",true);
				$_points++;
			
				update_user_meta($user_votes_id,$user_votes_id."_points",$_points);
				add_user_meta($user_votes_id,$user_votes_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),(wpqa_options("point_voting_answer") != ""?wpqa_options("point_voting_answer"):1),"+","voting_answer",$post_id,$id,"time" => current_time('timestamp')));
			
				$points_user = get_user_meta($user_votes_id,"points",true);
				update_user_meta($user_votes_id,"points",$points_user+(wpqa_options("point_voting_answer") != ""?wpqa_options("point_voting_answer"):1));
			}
	...
	function wpqa_following_you () {
		...
		if (isset($update)) {
			if ($active_points == "on") {
				...
				$_points = get_user_meta($following_you_id,$following_you_id."_points",true);
				$_points++;
				
				update_user_meta($following_you_id,$following_you_id."_points",$_points);
				add_user_meta($following_you_id,$following_you_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$point_following_me,"+","user_follow","","","time" => current_time('timestamp'),"user_id" => $user_id));
			}
	...
	function wpqa_following_not () {
		...
		if (is_array($following_me) && in_array($following_not_id,$following_me)) {
			...
			if ($active_points == "on") {
				...
				$_points = get_user_meta($following_not_id,$following_not_id."_points",true);
				$_points++;
				
				update_user_meta($following_not_id,$following_not_id."_points",$_points);
				add_user_meta($following_not_id,$following_not_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$point_following_me,"-","user_unfollow","","","time" => current_time('timestamp'),"user_id" => $user_id));
			}
	...
	function wpqa_add_point () {
		...
		if ($user_id != $get_post->post_author) {
			...
			if ($input_add_point == "") {
				...
				$_points++;
				update_user_meta($user_id,$user_id."_points",$_points);
				add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$input_add_point,"-","bump_question",$post_id,"time" => current_time('timestamp')));
				$points_user = get_user_meta($user_id,"points",true);
...
if (!function_exists('wpqa_report_q')) :
	function wpqa_report_q () {
		if (isset($_POST['wpqa_report_nonce']) && wp_verify_nonce($_POST['wpqa_report_nonce'],'wpqa_report_nonce')) {
			...
			wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),wpqa_options("email_template"),$email_title,$last_message_email);
			if ($user_id > 0) {
				...
				if ($active_trash_reports == "on" && $active_points == "on") {
					...
					if ($trash_reports_points > 0 && $points >= $trash_reports_points) {
						...
						$points_2 = ($reports_min_points > 0?get_user_meta(($post_author > 0?$post_author:$anonymously_user),"points",true):0);
...
if (!function_exists('wpqa_report_c')) :
	function wpqa_report_c () {
		if (isset($_POST['wpqa_report_nonce']) && wp_verify_nonce($_POST['wpqa_report_nonce'],'wpqa_report_nonce')) {
			...
			$email_title = ($email_title != ""?$email_title:__("Answer report","wpqa"));
			wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),wpqa_options("email_template"),$email_title,$last_message_email);
			if ($user_id > 0) {
				$active_trash_reports = wpqa_options("active_trash_reports");
				$active_points = wpqa_options("active_points");
				if ($active_trash_reports == "on" && $active_points == "on") {
					...
					if ($trash_reports_points > 0 && $points >= $trash_reports_points) {
						...
						$reports_min_points = (int)wpqa_options("reports_min_points");
						$points_2 = ($reports_min_points > 0?get_user_meta(($comment_user > 0?$comment_user:$anonymously_user),"points",true):0);
			...
			if (isset($the_best_answer) && $the_best_answer != "" && $the_best_answer > 0) {
				...
				if ($user_id != 0) {
					if ($user_id != $user_author && $active_points == "on") {
						...
						$_points++;
						update_user_meta($user_id,$user_id."_points",$_points);
						add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),(wpqa_options("point_best_answer") != ""?wpqa_options("point_best_answer"):5),"+","select_best_answer",$post_id,$comment_id,"time" => current_time('timestamp')));
						$points_user = get_user_meta($user_id,"points",true);
						update_user_meta($user_id,"points",$points_user+(wpqa_options("point_best_answer") != ""?wpqa_options("point_best_answer"):5));
					}
				...
				if ($point_back_option == "on" && $active_points == "on" && (is_super_admin($get_current_user_id) || ($user_id != $user_author && $user_author > 0) || ($user_id != $anonymously_user && $anonymously_user > 0))) {
					$point_back_number = wpqa_options("point_back_number");
					...
					if ($point_back == "yes" && ($user_author > 0 || $anonymously_user > 0)) {
						...
						$_points = get_user_meta($author_points,$author_points."_points",true);
						$_points++;
						update_user_meta($author_points,$author_points."_points",$_points);
						add_user_meta($author_points,$author_points."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),($what_point != ""?$what_point:wpqa_options("question_points")),"+","point_back",$post_id,$comment_id,"time" => current_time('timestamp')));
						$points_user = get_user_meta($author_points,"points",true);
						update_user_meta($author_points,"points",$points_user+($what_point != ""?$what_point:wpqa_options("question_points")));
			...
			if (isset($the_best_answer) && $the_best_answer != "" && $the_best_answer > 0 && $the_best_answer == $comment_id) {
				...
				if ($user_id != 0) {
					if ($user_id != $user_author && $active_points == "on") {
						...
						$_points++;
						update_user_meta($user_id,$user_id."_points",$_points);
						add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),(wpqa_options("point_best_answer") != ""?wpqa_options("point_best_answer"):5),"-","cancel_best_answer",$post_id,$comment_id,"time" => current_time('timestamp')));
						$points_user = get_user_meta($user_id,"points",true);
						update_user_meta($user_id,"points",$points_user-(wpqa_options("point_best_answer") != ""?wpqa_options("point_best_answer"):5));
					}
				...
				if ($point_back_option == "on" && $active_points == "on" && (is_super_admin($get_current_user_id) || ($user_id != $user_author && $user_author > 0) || ($user_id != $anonymously_user && $anonymously_user > 0))) {
					...
					if ($point_back == "yes" && ($user_author > 0 || $anonymously_user > 0)) {
						...
						$_points++;
						update_user_meta($author_points,$author_points."_points",$_points);
						add_user_meta($author_points,$author_points."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),($what_point != ""?$what_point:wpqa_options("question_points")),"-","point_removed",$post_id,$comment_id,"time" => current_time('timestamp')));
						$points_user = get_user_meta($author_points,"points",true);
						update_user_meta($author_points,"points",$points_user-($what_point != ""?$what_point:wpqa_options("question_points")));
					}
				...
				<ul>";
					if ($search_type == "answers" || $search_type == "comments") {
						...
						$args = array(
							...
							'order'          => "DESC",
							'search'         => '*'.$search_value.'*',
							'search_columns' => array('ID','user_login','user_nicename','user_email','user_url','display_name'),
							'number'         => $number,
							'fields'         => 'ID',
						);
