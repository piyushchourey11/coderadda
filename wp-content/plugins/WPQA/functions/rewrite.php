...
if (!function_exists('wpqa_redirect_pages')) :
	function wpqa_redirect_pages() {
		...
		if (wpqa_is_search()) {
			if (!empty($_GET['search']) || !empty($_GET['s']) || !empty($_GET['search_type']) || !empty($_GET['page']) || !empty($_GET['paged'])) {
...
if (!function_exists('wpqa_rewrite_tags')) :
	function wpqa_rewrite_tags() {
		add_rewrite_tag('%'.apply_filters('wpqa_search_id','search')                        .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_type','search_type')                        .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_buy_points','wpqa_buy_points')              .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_login','wpqa_login')                        .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_signup','wpqa_signup')                      .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_lost_password','wpqa_lost_password')        .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_add_questions','add_question')              .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_edit_questions','edit_question')            .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_add_posts','wpqa_add_post')                 .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_edit_posts','edit_post')                    .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_edit_comments','edit_comment')              .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_user_id','wpqa_user_id')                    .'%','([^/]+)');
		add_rewrite_tag('%'.apply_filters('wpqa_edit_id','edit')                            .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_followers','followers')                     .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_following','following')                     .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_notifications','notifications')             .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_activities','activities')                   .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_messages','messages')                       .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_questions','questions')                     .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_answers','answers')                         .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_best_answers','best_answers')               .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_points','points')                           .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_polls','polls')                             .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_asked','asked')                             .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_asked_questions','asked_questions')         .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_paid_questions','paid_questions')           .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_followed','followed')                       .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_favorites','favorites')                     .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_posts','posts')                             .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_comments','comments')                       .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_followers_questions','followers_questions') .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_followers_answers','followers_answers')     .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_followers_posts','followers_posts')         .'%','([1]{1,})');
		add_rewrite_tag('%'.apply_filters('wpqa_followers_comments','followers_comments')   .'%','([1]{1,})');
	}
endif;
/* Rewrite tags */
...
if (!function_exists('wpqa_rewrite_rules')) :
	function wpqa_rewrite_rules() {
		...
		$root_rule                     = '/([^/]+)/?$';
		$edit_rule                     = '/([^/]+)/' . $edit_slug  . '/?$';
		$page_rule                     = '/([^/]+)/' . $page_slug . '/?([0-9]{1,})/?$';
		
		$search_root_rule              = '/?$';
		$search_type_rule              = '/([^/]+)/([^/]+)/?$';
		$search_page_rule              = '/([^/]+)/([^/]+)/([0-9]{1,})/?$';
		
		$buy_points_root_rule          = '/?$';
		
		$login_root_rule               = '/?$';
		
		$signup_root_rule              = '/?$';
		
		$lost_password_root_rule       = '/?$';
		
		$add_questions_root_rule       = '/?$';
		
		$edit_questions_root_rule      = '/?$';
		
		$add_posts_root_rule           = '/?$';
		
		$edit_posts_root_rule          = '/?$';
		
		$edit_comments_root_rule       = '/?$';
		...
		// User profile rules
		$followers_rule                = '/([^/]+)/' . $user_followers_slug      . '/?$';
		$followers_page_rule           = '/([^/]+)/' . $user_followers_slug      . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$following_rule                = '/([^/]+)/' . $user_following_slug      . '/?$';
		$following_page_rule           = '/([^/]+)/' . $user_following_slug      . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$notifications_rule            = '/([^/]+)/' . $user_notifications_slug  . '/?$';
		$notifications_page_rule       = '/([^/]+)/' . $user_notifications_slug  . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$activities_rule               = '/([^/]+)/' . $user_activities_slug     . '/?$';
		$activities_page_rule          = '/([^/]+)/' . $user_activities_slug     . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$messages_rule                 = '/([^/]+)/' . $user_messages_slug     . '/?$';
		$messages_page_rule            = '/([^/]+)/' . $user_messages_slug     . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$questions_rule                = '/([^/]+)/' . $user_questions_slug      . '/?$';
		$questions_page_rule           = '/([^/]+)/' . $user_questions_slug      . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$answers_rule                  = '/([^/]+)/' . $user_answers_slug        . '/?$';
		$answers_page_rule             = '/([^/]+)/' . $user_answers_slug        . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$best_answers_rule             = '/([^/]+)/' . $best_answers_slug        . '/?$';
		$best_answers_page_rule        = '/([^/]+)/' . $best_answers_slug        . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$points_rule                   = '/([^/]+)/' . $user_points_slug         . '/?$';
		$points_page_rule              = '/([^/]+)/' . $user_points_slug         . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$polls_rule                    = '/([^/]+)/' . $user_polls_slug          . '/?$';
		$polls_page_rule               = '/([^/]+)/' . $user_polls_slug          . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$asked_rule                    = '/([^/]+)/' . $user_asked_slug          . '/?$';
		$asked_page_rule               = '/([^/]+)/' . $user_asked_slug          . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$asked_questions_rule          = '/([^/]+)/' . $asked_questions_slug     . '/?$';
		$asked_questions_page_rule     = '/([^/]+)/' . $asked_questions_slug     . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$paid_questions_rule           = '/([^/]+)/' . $paid_questions_slug     . '/?$';
		$paid_questions_page_rule      = '/([^/]+)/' . $paid_questions_slug     . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$followed_rule                 = '/([^/]+)/' . $user_followed_slug       . '/?$';
		$followed_page_rule            = '/([^/]+)/' . $user_followed_slug       . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$favorites_rule                = '/([^/]+)/' . $user_favorites_slug      . '/?$';
		$favorites_page_rule           = '/([^/]+)/' . $user_favorites_slug      . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$posts_rule                    = '/([^/]+)/' . $user_posts_slug          . '/?$';
		$posts_page_rule               = '/([^/]+)/' . $user_posts_slug          . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$comments_rule                 = '/([^/]+)/' . $user_comments_slug       . '/?$';
		$comments_page_rule            = '/([^/]+)/' . $user_comments_slug       . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$followers_questions_rule      = '/([^/]+)/' . $followers_questions_slug . '/?$';
		$followers_questions_page_rule = '/([^/]+)/' . $followers_questions_slug . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$followers_answers_rule        = '/([^/]+)/' . $followers_answers_slug   . '/?$';
		$followers_answers_page_rule   = '/([^/]+)/' . $followers_answers_slug   . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$followers_posts_rule          = '/([^/]+)/' . $followers_posts_slug     . '/?$';
		$followers_posts_page_rule     = '/([^/]+)/' . $followers_posts_slug     . '/' . $page_slug . '/?([0-9]{1,})/?$';
		$followers_comments_rule       = '/([^/]+)/' . $followers_comments_slug  . '/?$';
		$followers_comments_page_rule  = '/([^/]+)/' . $followers_comments_slug  . '/' . $page_slug . '/?([0-9]{1,})/?$';
		
		// User Pagination|Edit
		add_rewrite_rule( $user_slug . $followers_page_rule,                'index.php?' . $user_id  . '=$matches[1]&' . $user_followers_id      . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $followers_rule,                     'index.php?' . $user_id  . '=$matches[1]&' . $user_followers_id      . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $following_page_rule,                'index.php?' . $user_id  . '=$matches[1]&' . $user_following_id      . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $following_rule,                     'index.php?' . $user_id  . '=$matches[1]&' . $user_following_id      . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $notifications_page_rule,            'index.php?' . $user_id  . '=$matches[1]&' . $user_notifications_id  . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $notifications_rule,                 'index.php?' . $user_id  . '=$matches[1]&' . $user_notifications_id  . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $activities_page_rule,               'index.php?' . $user_id  . '=$matches[1]&' . $user_activities_id     . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $activities_rule,                    'index.php?' . $user_id  . '=$matches[1]&' . $user_activities_id     . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $messages_page_rule,                 'index.php?' . $user_id  . '=$matches[1]&' . $user_messages_id       . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $messages_rule,                      'index.php?' . $user_id  . '=$matches[1]&' . $user_messages_id       . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $questions_page_rule,                'index.php?' . $user_id  . '=$matches[1]&' . $user_questions_id      . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $questions_rule,                     'index.php?' . $user_id  . '=$matches[1]&' . $user_questions_id      . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $answers_page_rule,                  'index.php?' . $user_id  . '=$matches[1]&' . $user_answers_id        . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $answers_rule,                       'index.php?' . $user_id  . '=$matches[1]&' . $user_answers_id        . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $best_answers_page_rule,             'index.php?' . $user_id  . '=$matches[1]&' . $best_answers_id        . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $best_answers_rule,                  'index.php?' . $user_id  . '=$matches[1]&' . $best_answers_id        . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $points_page_rule,                   'index.php?' . $user_id  . '=$matches[1]&' . $user_points_id         . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $points_rule,                        'index.php?' . $user_id  . '=$matches[1]&' . $user_points_id         . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $polls_page_rule,                    'index.php?' . $user_id  . '=$matches[1]&' . $user_polls_id          . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $polls_rule,                         'index.php?' . $user_id  . '=$matches[1]&' . $user_polls_id          . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $asked_page_rule,                    'index.php?' . $user_id  . '=$matches[1]&' . $user_wpqa_asked        . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $asked_rule,                         'index.php?' . $user_id  . '=$matches[1]&' . $user_wpqa_asked        . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $asked_questions_page_rule,          'index.php?' . $user_id  . '=$matches[1]&' . $asked_questions_id     . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $asked_questions_rule,               'index.php?' . $user_id  . '=$matches[1]&' . $asked_questions_id     . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $paid_questions_page_rule,           'index.php?' . $user_id  . '=$matches[1]&' . $paid_questions_id      . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $paid_questions_rule,                'index.php?' . $user_id  . '=$matches[1]&' . $paid_questions_id      . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $followed_page_rule,                 'index.php?' . $user_id  . '=$matches[1]&' . $user_followed_id       . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $followed_rule,                      'index.php?' . $user_id  . '=$matches[1]&' . $user_followed_id       . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $favorites_page_rule,                'index.php?' . $user_id  . '=$matches[1]&' . $user_favorites_id      . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $favorites_rule,                     'index.php?' . $user_id  . '=$matches[1]&' . $user_favorites_id      . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $posts_page_rule,                    'index.php?' . $user_id  . '=$matches[1]&' . $user_posts_id          . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $posts_rule,                         'index.php?' . $user_id  . '=$matches[1]&' . $user_posts_id          . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $comments_page_rule,                 'index.php?' . $user_id  . '=$matches[1]&' . $user_comments_id       . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $comments_rule,                      'index.php?' . $user_id  . '=$matches[1]&' . $user_comments_id       . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $followers_questions_page_rule,      'index.php?' . $user_id  . '=$matches[1]&' . $followers_questions_id . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $followers_questions_rule,           'index.php?' . $user_id  . '=$matches[1]&' . $followers_questions_id . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $followers_answers_page_rule,        'index.php?' . $user_id  . '=$matches[1]&' . $followers_answers_id   . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $followers_answers_rule,             'index.php?' . $user_id  . '=$matches[1]&' . $followers_answers_id   . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $followers_posts_page_rule,          'index.php?' . $user_id  . '=$matches[1]&' . $followers_posts_id     . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $followers_posts_rule,               'index.php?' . $user_id  . '=$matches[1]&' . $followers_posts_id     . '=1',                              $priority );
		add_rewrite_rule( $user_slug . $followers_comments_page_rule,       'index.php?' . $user_id  . '=$matches[1]&' . $followers_comments_id  . '=1&' . $page_id . '=$matches[2]', $priority );
		add_rewrite_rule( $user_slug . $followers_comments_rule,            'index.php?' . $user_id  . '=$matches[1]&' . $followers_comments_id  . '=1',                              $priority );
...
if (!function_exists('wpqa_template_include')) :
	function wpqa_template_include ($new_template) {
		global $wp_query;
		if (wpqa_is_user_profile() || wpqa_is_search() || wpqa_is_buy_points() || wpqa_is_login() || wpqa_is_signup() || wpqa_is_lost_password() || wpqa_is_add_questions() || wpqa_is_edit_questions() || wpqa_is_add_posts() || wpqa_is_edit_posts() || wpqa_is_edit_comments()) {
			global $wp_query,$wpqa;
...
/* Check if user add question to user */
...
if (!function_exists('wpqa_profile_title')) :
	function wpqa_profile_title() {
		if (wpqa_is_user_followers()) {
			return esc_html__("Followers","wpqa");
		}else if (wpqa_is_user_following()) {
			return esc_html__("Following","wpqa");
		}else if (wpqa_is_user_notifications()) {
			return esc_html__("Notifications","wpqa");
		}else if (wpqa_is_user_activities()) {
			return esc_html__("Activities","wpqa");
		}else if (wpqa_is_user_messages()) {
			return esc_html__("Messages","wpqa");
		}else if (wpqa_is_user_questions()) {
			return esc_html__("Questions","wpqa");
		}else if (wpqa_is_user_answers()) {
			return esc_html__("Answers","wpqa");
		}else if (wpqa_is_best_answers()) {
			return esc_html__("Best Answers","wpqa");
		}else if (wpqa_is_user_points()) {
			return esc_html__("Points","wpqa");
		}else if (wpqa_is_user_polls()) {
			return esc_html__("Polls","wpqa");
		}else if (wpqa_is_user_asked()) {
			return esc_html__("Asked Questions","wpqa");
		}else if (wpqa_is_asked_questions()) {
			return esc_html__("Waiting Questions","wpqa");
		}else if (wpqa_is_paid_questions()) {
			return esc_html__("Paid Questions","wpqa");
		}else if (wpqa_is_user_followed()) {
			return esc_html__("Followed","wpqa");
		}else if (wpqa_is_user_favorites()) {
			return esc_html__("Favorites","wpqa");
		}else if (wpqa_is_user_posts()) {
			return esc_html__("Posts","wpqa");
		}else if (wpqa_is_user_comments()) {
			return esc_html__("Comments","wpqa");
		}else if (wpqa_is_followers_questions()) {
			return esc_html__("Followers Questions","wpqa");
		}else if (wpqa_is_followers_answers()) {
			return esc_html__("Followers Answers","wpqa");
		}else if (wpqa_is_followers_posts()) {
			return esc_html__("Followers Posts","wpqa");
		}else if (wpqa_is_followers_comments()) {
			return esc_html__("Followers Comments","wpqa");
		}else if (wpqa_is_user_edit_profile() && wpqa_is_user_owner()) {
			return esc_html__("Edit profile","wpqa");
		}
	}
endif;
/* Parse query */
...
if (!function_exists('wpqa_parse_query')) :
	function wpqa_parse_query( $posts_query ) {
		...
		if ( !empty( $is_user ) || isset($posts_query->query_vars[apply_filters('wpqa_search_id','search')]) || isset($posts_query->query_vars[apply_filters('wpqa_buy_points','wpqa_buy_points')]) || isset($posts_query->query_vars[apply_filters('wpqa_login','wpqa_login')]) || isset($posts_query->query_vars[apply_filters('wpqa_signup','wpqa_signup')]) || isset($posts_query->query_vars[apply_filters('wpqa_lost_password','wpqa_lost_password')]) || isset($posts_query->query_vars[apply_filters('wpqa_add_questions','add_question')]) || isset($posts_query->query_vars[apply_filters('wpqa_edit_questions','edit_question')]) || isset($posts_query->query_vars[apply_filters('wpqa_add_posts','wpqa_add_post')]) || isset($posts_query->query_vars[apply_filters('wpqa_edit_posts','edit_post')]) || isset($posts_query->query_vars[apply_filters('wpqa_edit_comments','edit_comment')]) ) {
		...
		if ( !empty( $is_user ) ) {
			...
			$profile_type = wpqa_options("profile_type");
			$posts_query->set('author_name',trim(urldecode(esc_attr($profile_type == "login"?$the_user->user_login:$the_user->user_nicename))));
			$posts_query->set('wpqa_user',trim(urldecode(esc_attr($profile_type == "login"?$the_user->user_login:$the_user->user_nicename))));
...
if (!function_exists('wpqa_search')) :
	function wpqa_search($encode = "") {
		...
		if ($encode == "encode") {
			$search_value = esc_attr(urlencode(urldecode(trim($search_value))));
		}else {
			$search_value = esc_attr(urldecode(trim($search_value)));
		}
...
if (!function_exists('wpqa_search_terms')) :
	function wpqa_search_terms( $passed_terms = '' ) {
		if ( !empty( $passed_terms ) ) {
			...
			$search_terms = esc_attr(get_query_var(apply_filters('wpqa_search_id','search')));
		}
...
if (!function_exists('wpqa_search_link')) :
	function wpqa_search_link($query = "",$type = "",$page = "") {
		...
		if ($wp_rewrite->using_permalinks()) {
			$search_slug = wpqa_options("search_slug");
			$url = home_url('/'.$search_slug.'/'.($query != ""?$query.'/':'').($type != ""?$type.'/':'').($paged != ""?esc_attr($paged).'/':'').($user_filter != ''?'?user_filter='.$user_filter:''));
		}else {
			$url = esc_url(add_query_arg(array(apply_filters('wpqa_search_id','search') => $query,apply_filters('wpqa_type','search_type') => $type,'page' => esc_attr($paged),array('user_filter' => $user_filter)),home_url()));
		}
		return esc_url(apply_filters( 'wpqa_filter_search_link', $url ));
	}
endif;
/* Search type */
...
function wpqa_get_search_permalink() {
	...
	if ( $wp_rewrite->using_permalinks() ) {
		...
		$url = esc_url(add_query_arg( array( apply_filters('wpqa_search_id','search') => wpqa_search() ), home_url( '/' ) ));
	}
...
function wpqa_buy_points_permalink() {
	...
	if ( $wp_rewrite->using_permalinks() ) {
		...
		$url = esc_url(add_query_arg( array( apply_filters('wpqa_buy_points','wpqa_buy_points') => (isset($get_query_var) && $get_query_var != ""?$get_query_var:"") ), home_url( '/' ) ));
	}
...
function wpqa_login_permalink() {
	...
	if ( $wp_rewrite->using_permalinks() ) {
		...
		$url = esc_url(add_query_arg( array( apply_filters('wpqa_login','wpqa_login') => (isset($get_query_var) && $get_query_var != ""?$get_query_var:"") ), home_url( '/' ) ));
	}
...
function wpqa_signup_permalink() {
	...
	if ( $wp_rewrite->using_permalinks() ) {
		...
		$url = esc_url(add_query_arg( array( apply_filters('wpqa_signup','wpqa_signup') => (isset($get_query_var) && $get_query_var != ""?$get_query_var:"") ), home_url( '/' ) ));
	}
...
function wpqa_lost_password_permalink() {
	...
	if ( $wp_rewrite->using_permalinks() ) {
		...
		$url = esc_url(add_query_arg( array( apply_filters('wpqa_lost_password','wpqa_lost_password') => (isset($get_query_var) && $get_query_var != ""?$get_query_var:"") ), home_url( '/' ) ));
	}
...
function wpqa_add_post_permalink() {
	...
	if ( $wp_rewrite->using_permalinks() ) {
		...
		$url = esc_url(add_query_arg( array( apply_filters('wpqa_add_posts','wpqa_add_post') => (isset($get_query_var) && $get_query_var != ""?$get_query_var:"") ), home_url( '/' ) ));
	}
	return apply_filters( 'wpqa_add_post_url', $url );
}
/* Get add question url */
function wpqa_add_question_permalink($user = '') {
	global $wp_rewrite;
	$add_questions_slug = wpqa_options("add_questions_slug");
	$get_query_var      = '';
	if (wpqa_is_user_profile() && $user == "user") {
		$wpqa_user_id = (int)get_query_var(apply_filters('wpqa_user_id','wpqa_user_id'));
		$profile_type = wpqa_options("profile_type");
		if ($profile_type == "login") {
	...
	if ( $wp_rewrite->using_permalinks() ) {
		...
		$url = esc_url(add_query_arg( array( apply_filters('wpqa_add_questions','add_question') => (isset($get_query_var) && $get_query_var != ""?$get_query_var:"") ), home_url( '/' ) ));
	}
	return apply_filters( 'wpqa_add_question_url', $url );
}
/* Get user at add question page */
...
function wpqa_get_profile_permalink( $user_id = 0,$type = 'questions' ) {
	...
	return apply_filters( 'wpqa_get_'.$type.'_permalink', $url, $user_id );
}
/* Filter profile url */
...
if (!function_exists('wpqa_author_link')) :
	function wpqa_author_link($link,$user_id,$user_nicename) {
		...
		if ( is_string( $early_profile_url ) )
			return $early_profile_url;
		if ( $wp_rewrite->using_permalinks() ) {
			$link = wpqa_options("profile_slug") . '/%' . apply_filters('wpqa_user_id','wpqa_user_id') . '%';
			$profile_type = wpqa_options("profile_type");
			...
			$link = esc_url(add_query_arg( array( apply_filters('wpqa_user_id','wpqa_user_id') => $user_id ), home_url( '/' ) ));
		}
...
function wpqa_the_title($title) {
	if (wpqa_is_search()) {
		...
		$title['title'] = esc_html__('Add question','wpqa');
	}else if (wpqa_is_edit_questions()) {
