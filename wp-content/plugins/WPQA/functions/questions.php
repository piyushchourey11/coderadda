						...
						if (is_super_admin($another_user_id) && ($private_question == "on" || $private_question == 1) && (($another_user_id != $anonymously_user && $anonymously_user > 0) || ($another_user_id != $not_user && $not_user > 0))) {
							...
							if ($not_user != $another_user_id && (empty($user_group) || (is_array($user_group) && is_array($get_capabilities) && in_array(key($get_capabilities),$user_group)))) {
								...
								if ($yes_private == 1) {
									...
									wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),esc_attr($value->user_email),$email_title,$last_message_email);
								}
							}
						}
					}
...
if (!function_exists('wpqa_add_admin_page_reports')) :
	function wpqa_add_admin_page_reports() {
		...
		if ($active_reports == "on") {
			...
			$wpqa_page = add_menu_page(esc_html__('Reports','wpqa'), esc_html__('Reports','wpqa').' <span class="count_report_new awaiting-mod count-'.$count_lasts.'"><span class="count_lasts">'.$count_lasts.'</span></span>' ,'manage_options', 'r_questions' , 'wpqa_r_questions','dashicons-email-alt');
			add_submenu_page( 'r_questions', esc_html__('Questions','wpqa'), esc_html__('Questions','wpqa').' <span class="count_report_new awaiting-mod count-'.$count_report_new.'"><span class="count_report_question_new">'.$count_report_new.'</span></span>', 'manage_options', 'r_questions', 'wpqa_r_questions' );
			add_submenu_page( 'r_questions', esc_html__('Answers','wpqa'), esc_html__('Answers','wpqa').' <span class="count_report_new awaiting-mod count-'.$count_report_answer_new.'"><span class="count_report_answer_new">'.$count_report_answer_new.'</span></span>', 'manage_options', 'r_answers', 'wpqa_r_answers' );
		}
...
if (!function_exists('wpqa_reports')) :
	function wpqa_reports ($type = "") {
		...
		<div class="wrap">
			...
			if (is_array($wpqa_option_array)) {
				foreach ($wpqa_option_array as $key => $value) {
					$ask_one_option = get_option("ask_option".$type."_".$value);
					if (isset($ask_one_option) && !empty($ask_one_option)) {
						update_option("report_option".$type."_".$value,$ask_one_option);
		...
		if ($pay_to_sticky == "on") {
			...
			if (($end_sticky_time == "" || ($end_sticky_time != "" && $end_sticky_time < strtotime(date("Y-m-d"))))) {
				...
				if ($user_id > 0 && isset($_POST["process"]) && $_POST["process"] == "sticky") {
					...
					if (isset($_POST["points"]) && $_POST["points"] > 0) {
						...
						if ($points_price <= $points_user) {
							...
							add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$points_price,"-","sticky_points","","time" => current_time('timestamp')));

							update_user_meta($user_id,"points",$points_user-$points_price);
							$_SESSION['wpqa_session'] = '<div class="alert-message success"><i class="icon-check"></i><p>'.esc_html__("You have just stickied the question by points.","wpqa").'</p></div>';
						}else {
				...
				if ((($anonymously_user > 0 && $user_id == $anonymously_user) || ($post_author > 0 && $user_id == $post_author)) && (($end_sticky_time != "" && $end_sticky_time < strtotime(date("Y-m-d"))) || (!is_sticky())) && isset($_allow_to_sticky) && (int)$_allow_to_sticky < 1 && $pay_to_sticky == "on") {
					$pay_sticky_payment = $last_payment = (int)wpqa_options("pay_sticky_payment");
	...
	function wpqa_question_list_details($post,$user_id,$anonymously_user,$question_edit,$question_delete,$question_close,$closed_question,$active_reports,$active_logged_reports) {
		echo '<ul class="question-link-list">';
			...
			if ($active_reports == "on" && ((is_user_logged_in() && ($user_id != $post->post_author && $post->post_author > 0) || $post->post_author == 0 || $anonymously_user == 0 || ($anonymously_user != "" && $anonymously_user != $user_id && $user_id > 0)) || (!is_user_logged_in() && $active_logged_reports != "on"))) {
				echo '<li><a class="report_q" href="'.esc_attr($post->ID).'"><i class="icon-attention"></i>'.esc_html__("Report","wpqa").'</a></li>';
			}
		echo '</ul>';
	}
endif;?>