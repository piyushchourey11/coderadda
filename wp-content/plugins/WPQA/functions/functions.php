...
/* Send Email */
function wpqa_html_email() {
	return "text/html";
}
add_filter('wp_mail_content_type','wpqa_html_email');
if (!function_exists('wpqa_sendEmail')) :
	function wpqa_sendEmail($fromEmail,$fromEmailName,$toEmail,$subject,$message) {
	...
	function wpqa_show_notifications($whats_type_result,$show_date = "") {
		...
		if (!empty($whats_type_result["post_id"]) && empty($whats_type_result["comment_id"]) && $get_post_status != "trash" && isset($get_the_permalink) && $get_the_permalink != "") {
			...
			if ($whats_type_result["text"] == "poll_question") {
				...
				$output .= esc_html__("added you question to favorites.","wpqa");
				...
				<ul>
					<?php foreach ($comments as $comment) {
						...
						<li>
							...
							<?php }?>
							<div>
								<?php echo ($comment->user_id != ""?"<a href='".esc_url($user_profile_page)."' target='_blank'>":"").strip_tags($comment->comment_author).($comment->user_id != ""?"</a>":"") ." ". ($post_or_question == "question"?esc_html__("added an answer","wpqa"):esc_html__("added a comment","wpqa")) ?> <span class="question-title"><a href="<?php echo get_permalink($comment->comment_post_ID);?>#comment-<?php echo esc_attr($comment->comment_ID);?>"><?php echo wp_html_excerpt($comment->comment_content,$comment_excerpt);?></a></span>
							...
							$out .= '<li class="widget-posts-';if (is_sticky()) {$out .= 'sticky';}else if ($what_post == "google") {$out .= 'google';}else if ($what_post == "audio") {$out .= 'volume-up';}else if ($what_post == "video") {if ($video_type == 'youtube') {$out .= 'youtube';}else if ($video_type == 'vimeo') {$out .= 'vimeo';}else if ($video_type == 'daily' || $video_type == 'embed' || $video_type == 'html5' || $video_type == 'facebook') {$out .= 'daily';}}else if ($what_post == "slideshow") {$out .= 'slideshow';}else if ($what_post == "quote") {$out .= 'quote';}else if ($what_post == "link") {$out .= 'link';}else if ($what_post == "soundcloud") {$out .= 'soundcloud';}else if ($what_post == "twitter") {$out .= 'twitter';}else if ($what_post == "facebook") {$out .= 'facebook';}else if ($what_post == "instagram") {$out .= 'instagram';}else {if (has_post_thumbnail()) {$out .= 'image';}else {$out .= 'text';}}$out .= (has_post_thumbnail()?'':' widget-no-img').($display_comment || ($post_style == "style_2" && $display_date == "on")?'':' widget-no-meta').'">';
								...
								if ($post_style == "style_2" && $display_video == "on" && ($what_post == "video" || $video_description == "on")) {
									...
									if (has_post_thumbnail()) {
										$out .= wpqa_get_aq_resize_img($img_width,$img_height);
									}else {
...
if (!function_exists('wpqa_author')) :
	function wpqa_author($author_id,$author_page = "",$owner = "",$type_post = "",$widget = "",$class = "",$cover = "") {
		if (isset($author_id) && $author_id > 0) {
			if ($cover == "") {
				...
				if ($author_page == "grid" || $author_page == "grid_pop" || $author_page == "small" || $author_page == "simple_follow" || $author_page == "columns" || $author_page == "columns_pop") {
					/* questions */
					$questions_count = wpqa_count_posts_by_user($author_id,"question");
					/* answers */
				...
				<div class="post-inner">';
					...
					$out .= '<div class="user-content">
						...
						if ($author_page != "grid" && $author_page != "grid_pop" && $author_page != "small" && $author_page != "simple_follow" && $author_page != "columns" && $author_page != "columns_pop") {
							...
							if (($user_email && isset($follow_email) && $follow_email == "on") || $facebook || $twitter || $linkedin || $google || $youtube || $vimeo || $pinterest || $instagram || ($cover == "" && (!isset($message_button) || (isset($message_button) && $message_button == "")))) {
								$out .= '<div class="social-ul">
									<ul>';
										...
										if ($google) {
											$out .= '<li class="social-google"><a title="Google+" class="tooltip-n" href="'.esc_url($google).'" target="_blank"><i class="icon-gplus"></i></a></li>';
										}
										if ($linkedin) {
...
if (!function_exists('wpqa_get_badge')) :
	function wpqa_get_badge($author_id,$return = "") {
		...
		if ($badges_style == "by_groups_points") {
			if ($author_id > 0) {
				...
				if (isset($badges_groups_points) && is_array($badges_groups_points)) {
					...
					$badge_key = $last_key;
					if ($return == "color") {
						$badge_color = $badges_points[$badge_key]["badge_color"];
						return $badge_color;
					}else if ($return == "name") {
						$badge_name = $badges_points[$badge_key]["badge_name"];
						return $badge_name;
					}else {
						return '<span class="badge-span" style="background-color: '.$badges_points[$badge_key]["badge_color"].'">'.$badges_points[$badge_key]["badge_name"].'</span>';
					}
				}
			}
			...
			if ($author_id > 0) {
				...
				if (isset($badges_groups) && is_array($badges_groups)) {
					...
					if ($return == "color") {
						$badge_color = $badges_groups[$found_key]["badge_color"];
						return $badge_color;
					}else if ($return == "name") {
						return $user_group;
					}else {
			...
			if ($author_id > 0 && $active_points == "on") {
				...
				if (isset($badges) && is_array($badges)) {
					...
					$badge_key = $key+1;
					if ($return == "color") {
						$badge_color = $badges[$badge_key]["badge_color"];
						return $badge_color;
					}else if ($return == "name") {
						$badge_name = $badges[$badge_key]["badge_name"];
						return $badge_name;
					}else {
						return '<span class="badge-span" style="background-color: '.$badges[$badge_key]["badge_color"].'">'.$badges[$badge_key]["badge_name"].'</span>';
					}
				}
			}
		}
...
if (!function_exists('wpqa_count_posts_by_user')) :
	function wpqa_count_posts_by_user( $user_id, $post_type = 'post', $post_status = "publish" ) {
		global $wpdb;
	...
	function wpqa_user_table_row( $val, $column_name, $user_id ) {
		...
		switch ($column_name) {
			...
			case 'phone' :
				...
				break;
			case 'country' :
				$get_countries = wpqa_get_countries();
			...
			<table border="0" width="100%" cellspacing="0" cellpadding="0" bgcolor="#F4F3F4">
				...
				<td style="padding:15px">
					<center>
						...
						<tr>
						<td style="line-height:32px;padding:30px 30px 20px;text-align:center;background-color:'.$background_email.';" valign="baseline"><a href="'.esc_url(home_url('/')).'" target="_blank">'.($logo_email_template != ''?'<img src="'.$logo_email_template.'" alt="'.$logo_email_template.'">':'').'</a></td>
						</tr>
		...
		if (isset($following_me) && is_array($following_me)) {
			foreach ($following_me as $key => $value) {
				...
				if ($active_points == "on") {
					...
					$_points = get_user_meta($value,$value."_points",true);
					$_points++;
					
					update_user_meta($value,$value."_points",$_points);
					add_user_meta($value,$value."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$point_following_me,"-","delete_follow_user","","","time" => current_time('timestamp')));
				}
...
if (!function_exists('wpqa_delete_post')) :
	function wpqa_delete_post() {
		if (isset($_GET['wpqa_delete_nonce']) && wp_verify_nonce($_GET['wpqa_delete_nonce'],'wpqa_delete_nonce') && !is_admin() && isset($_GET["delete"]) && $_GET["delete"] != "") {
			...
			if (isset($post_id) && $post_id != "" && isset($get_post->post_status) && $get_post->post_status == "publish" && ($post_type == "post" || $post_type == "question")) {
				...
				if ('publish' == $new_status && $post->post_type == "message") {
					if ($post_approved_before != "yes") {
						...
						if ($send_email_message == "on") {
							...
							$email_title = ($email_title != ""?$email_title:__("New message","wpqa"));
							wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),esc_attr($user->user_email),$email_title,$last_message_email);
						}
					}
				}
	    	...
	    	<div id="comment-<?php echo esc_attr($comment_id);?>" class="comment-body clearfix">
	            ...
	            <div class="comment-text">
	                	...
	                	<div class="comment-meta">
	                    	<div class="comment-author">
	                        	...
	                        	if ($comment->user_id != 0) {
	                        		...
	                        		do_action("wpqa_get_badge",$comment->user_id);
	                        	}
	                        	if ($profile_credential != "") {?>
	                        		<span class="profile-credential"><?php stripslashes(sanitize_textarea_field($profile_credential))?></span>
	                        	<?php }?>
	                    	</div>
	                        <a href="<?php echo get_comment_link($comment_id); ?>" class="comment-date" itemprop="dateCreated" datetime="<?php echo get_comment_date(false,$comment_id)?>">
	                        	<?php if (isset($its_question) && $its_question == "question") {
	                        		...
	                        		printf(esc_html__('answer on %1$s at %2$s','wpqa'),get_comment_date(false,$comment_id),wpqa_get_comment_time(false,false,false,$comment));
	                        	}else {
	                        		echo ($comment->comment_parent > 0?esc_html__("Replied to","wpqa"):esc_html__("Added a","wpqa"))." ";
	                        		printf(esc_html__('comment on %1$s at %2$s','wpqa'),get_comment_date(false,$comment_id),wpqa_get_comment_time(false,false,false,$comment));
	                        	}?>
	                        </a> 
	                ...
	                <div class="text">
	                	...
	                	<div<?php echo (isset($its_question) && $its_question == "question"?" itemprop='text'":"")?>>
	                		...
	                		if ($active_vote == "on" && $active_vote_answer == "on") {
	                			if ($owner == false) {?>
		                			<ul class="question-vote answer-vote<?php echo ($show_dislike_answers != "on"?" answer-vote-dislike":"")?>">
		                				...
		                				<li class="vote_result" itemprop="upvoteCount"><?php echo ($comment_vote != ""?wpqa_count_number($comment_vote):0)?></li>
	                	...
	                	<ul class="comment-reply comment-reply-main">
	                	    ...
	                	    $share_google   = (isset($comment_share["share_google"]["value"])?$comment_share["share_google"]["value"]:"");
	                	    $share_whatsapp = (isset($comment_share["share_whatsapp"]["value"])?$comment_share["share_whatsapp"]["value"]:"");
	                	    if ($share_facebook == "share_facebook" || $share_twitter == "share_twitter" || $share_google == "share_google" || $share_whatsapp == "share_whatsapp") {?>
	                	    	<li class="comment-share question-share question-share-2">
	                	    		<i class="icon-share"></i>
	                	    		<?php esc_html_e("Share","wpqa");
	                	    		wpqa_share($comment_share,$share_facebook,$share_twitter,$share_google,$share_whatsapp,"style_2",$comment_id);?>
	                	    	</li>
	                	    <?php }
		                	    ...
		                	    <li class="question-list-details comment-list-details">
									...
									<ul>
				                	    ...
				                	    if (($can_delete_comment == "on" && $comment->user_id == $user_get_current_user_id && $comment->user_id > 0 && $user_get_current_user_id > 0) || current_user_can('edit_comment',$comment_id) || is_super_admin($user_get_current_user_id)) {
				                	    	echo "<li><a class='delete-comment".(isset($its_question) && $its_question == "question"?' delete-answer':'')."' href='".esc_url(add_query_arg(array('delete_comment' => $comment_id,"wpqa_delete_nonce" => wp_create_nonce("wpqa_delete_nonce")),get_permalink($comment->comment_post_ID)))."'><i class='icon-trash'></i>".esc_html__("Delete","wpqa")."</a></li>";
				                	    }
			                	    	if (isset($its_question) && $its_question == "question" && $active_reports == "on" && ((is_user_logged_in() && $comment->user_id != $user_get_current_user_id && $comment->user_id != 0 && $user_get_current_user_id != 0) || (!is_user_logged_in() && $active_logged_reports != "on"))) {
			                	    		if ($owner == false) {?>
				                	    		<li><a class="report_c" href="<?php echo esc_attr($comment_id)?>"><i class="icon-attention"></i><?php esc_html_e("Report","wpqa")?></a></li>
				                	    	<?php }
				                	    }?>
				               		</ul>
				               	</li>
...
if (!function_exists('wpqa_breadcrumbs')) :
	function wpqa_breadcrumbs ($text = "",$breadcrumb_right = true) {
	    ...
	    if (isset($text) && $text != "") {
	    	...
	    	echo ($before . esc_html__('Add question', "wpqa") . ($wpqa_add_question_user > 0?" ".esc_html__("to","wpqa")." ".$display_name:"") . $after);
		...
		if ($post_type == "question") {
			...
			if ($remove_best_answer_stats == "on" && $active_points == "on") {
				...
				if ($user_id > 0) {
					...
					$point_add_question = ($point_add_question != ""?$point_add_question:5);
					
					$_points = get_user_meta($user_id,$user_id."_points",true);
					$_points++;
					update_user_meta($user_id,$user_id."_points",$_points);
					add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$point_add_question,"-","delete_question","time" => current_time('timestamp')));
					$points_user = get_user_meta($user_id,"points",true);
					update_user_meta($user_id,"points",$points_user-$point_add_question);
				}
			}
		}
...
if (!function_exists('wpqa_head_content')) :
	function wpqa_head_content($login = "",$its_not_login = false) {
		if ($login != "login") {
			...
			if (is_user_logged_in() && $confirm_email == "on") {
			...
			if (isset($_POST["form_type"]) && (($_POST["form_type"] == "add_question" && isset($_POST["question_popup"]) && $_POST["question_popup"] == "popup") || ($_POST["form_type"] == "add_post" && isset($_POST["post_popup"]) && $_POST["post_popup"] == "popup") || ($_POST["form_type"] == "add_message" && isset($_POST["message_popup"]) && $_POST["message_popup"] == "popup") || $_POST["form_type"] == "wpqa-signup" || $_POST["form_type"] == "wpqa-login" || $_POST["form_type"] == "wpqa-forget")) {?>
				<script type="text/javascript">
					jQuery(document).ready(function($) {
						...
						<?php $form_type = (isset($_POST["form_type"])?esc_html($_POST["form_type"]):"");
		...
		if ($confirm_email != "yes" && $login != "login") {?>
			<div class="panel-pop" id="wpqa-question" data-width="690">
				<i class="icon-cancel"></i>
				<div class="panel-pop-content">
					<?php echo do_shortcode("[wpqa_question popup='popup']");?>
				</div><!-- End panel-pop-content -->
				<?php $pay_ask = wpqa_options("pay_ask");
			...
			if ($ask_question_to_users == "on" && wpqa_is_user_profile()) {?>
				<div class="panel-pop" id="wpqa-question-user" data-width="690">
					<i class="icon-cancel"></i>
					<div class="panel-pop-content">
			...
			if ($active_post_popup == "on") {
				$register = false;?>
				<div class="panel-pop" id="wpqa-post" data-width="690">
					<i class="icon-cancel"></i>
					<div class="panel-pop-content">
			...
			$active_message = wpqa_options("active_message");
			if ($active_message == "on" && wpqa_is_user_profile()) {
				$received_message = "";
				$user_block_message = array();
				$send_message_no_register = wpqa_options("send_message_no_register");
				$wpqa_user_id = (int)get_query_var(apply_filters('wpqa_user_id','wpqa_user_id'));
				...
				if (((!is_user_logged_in() && $send_message_no_register == "on") || (is_user_logged_in() && (empty($user_block_message) || (isset($user_block_message) && is_array($user_block_message) && !in_array(get_current_user_id(),$user_block_message))) && ($block_message != "on" || is_super_admin($wpqa_user_id)) && ($received_message == "" || $received_message == "on")))) {
					$register = false;?>
					<div class="panel-pop" id="wpqa-message" data-width="690">
						<i class="icon-cancel"></i>
						<div class="panel-pop-content">
...
if (!function_exists('wpqa_header_content')) :
	function wpqa_header_content($args = array()) {
		...
		if ($pay_ask == "on" || $pay_to_sticky == "on" || $buy_points_payment == "on") {
						...
						if (isset($item_transaction)) {
							if (isset($last_payments) && $last_payments == $item_transaction) {
								...
								if ($item_no == "pay_sticky") {
									...
									$buy_points = get_user_meta($user_id,"buy_points",true);
									$_points = get_user_meta($user_id,$user_id."_points",true);
									$_points++;
								
									update_user_meta($user_id,$user_id."_points",$_points);
									add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$buy_points,"+","buy_points",$id,"time" => current_time('timestamp')));

									$points_user = get_user_meta($user_id,"points",true);
									update_user_meta($user_id,"points",$points_user+$buy_points);
									...
									if ($_allow_to_ask == "" || !$_allow_to_ask) {
										$_allow_to_ask = 0;
									}
									$_allow_to_ask++;
								...
								wpqa_sendEmail($email_template,$first_name,$email_template,$email_title,$last_message_email);
								if ($payer_email != $email_template) {
									wpqa_sendEmail($email_template,$first_name,$payer_email,$email_title,$last_message_email);
								}
								$_SESSION['wpqa_session'] = '<div class="alert-message success"><i class="icon-check"></i><p>'.$payment_success.', '.sprintf(esc_html__("Your transaction id %s, Please copy it.","wpqa"),$item_transaction).'</p></div>';
...
if (!function_exists('wpqa_pagination')) :
	function wpqa_pagination($args = array(),$max_num_pages = '',$query = '') {
		...
		$defaults = array(
			'base' => (esc_url(add_query_arg($page_paged,'%#%'))),
			'format' => '',
			'total' => $max_num_pages,
		...
		/* If we're in BuddyPress,use the default "unpretty" URL structure. */
			if (class_exists('BP_Core_User')) {
				...
				if (!empty($search_permastruct))
					$defaults['base'] = esc_url(add_query_arg('page','%#%'));
			}
		}
									...
									if (page_url != undefined) {
										...
										jQuery("<div>").load(page_url, function() {
											...
											$new.imagesLoaded( function() {
												...
												if (jQuery(".post-section-new-"+n+".question-2-columns").length) {
													jQuery(".post-section-new-"+n+".question-2-columns").matchHeight();
													jQuery(".post-section-new-"+n+".question-2-columns > .article-question").matchHeight();
												}
...
if (!function_exists('wpqa_share')) :
	function wpqa_share($post_share,$share_facebook,$share_twitter,$share_google,$share_whatsapp,$share_style = "style_1",$comment_id = "") {
		if ($share_facebook == "share_facebook" || $share_twitter == "share_twitter" || $share_google == "share_google" || $share_whatsapp == "share_whatsapp") {
			$url = urlencode(get_permalink().($comment_id > 0?"#comment-".$comment_id:""));
			$desc = esc_js(wp_html_excerpt(strip_shortcodes(($comment_id > 0?get_comment_text($comment_id):get_the_content())),160));
			$image_for_share = ($comment_id > 0?wp_get_attachment_url(get_comment_meta($comment_id,'featured_image',true),"full"):wpqa_image_for_share());
			$img = ($image_for_share != ""?urlencode($image_for_share):"");
				...
				<ul>
					<?php foreach ($post_share as $key => $value) {
						if ($share_facebook == "share_facebook" && isset($value["value"]) && $value["value"] == "share_facebook") {?>
							<li class="share-facebook"><a href="http://www.facebook.com/sharer.php?u=<?php echo ($url)?>"><i class="icon-facebook"></i><?php echo ($share_style == "style_2"?esc_html__("Share on","wpqa")." ":"").esc_html__('Facebook', "wpqa")?></a></li>
						...
						<?php }else if ($share_google == "share_google" && isset($value["value"]) && $value["value"] == "share_google") {?>
							<li class="share-gplus"><a href="https://plus.google.com/share?url=<?php echo ($url);?>"><i class="icon-gplus"></i><?php echo ($share_style == "style_2"?esc_html__("Share on","wpqa")." ".esc_html__('Google plus', "wpqa"):"")?></a></li>
...
if (!function_exists('wpqa_image_url_id')) :
	function wpqa_image_url_id($url_id) {
		...
		if (!isset($image)) {
			if (is_array($url_id)) {
				if (isset($url_id['url']) && $url_id['url'] != '') {
...
if (!function_exists('wpqa_get_payment_coupons')) :
	function wpqa_get_payment_coupons ($user_id,$popup = false,$post_id = 0,$days_sticky = 0,$sticky_or_ask = "ask_question",$points = 0,$price = 0,$package_name = "") {
		$output = '';
		...
		if ($sticky_or_ask == "sticky") {
			...
			$item_number_1 = $points;
			$return_url = get_the_permalink();
			$item_process = "sticky";
		}else if ($sticky_or_ask == "points") {
			...
			$payment_description = esc_attr__("Ask a new question","wpqa");
			$item_number = "pay_ask";
			$return_url = wpqa_add_question_permalink();
			$item_process = "ask";
		}

		$payment_by_points = wpqa_options($payment_by_points);
		if ($payment_by_points == "points") {
			$points_price = (int)wpqa_options($points_price);
			if ($sticky_or_ask == "sticky") {
			...
			$pay_payment = $last_payment = (int)($price > 0?$price:wpqa_options($payment_option));
			...
			if ($sticky_or_ask == "sticky") {
				$output .= '<div class="alert-message success"><i class="icon-check"></i><p>'.sprintf(esc_html__("Please make a payment to allow to be able to sticky the question %s For %s days.","wpqa"),' "'.$last_payment." ".$currency_code.'"',$days_sticky).'</p></div>';
			}else if ($sticky_or_ask == "points") {
				$output .= '<div class="alert-message success"><i class="icon-check"></i><p>'.sprintf(esc_html__("Please make a payment to buy %s points %s.","wpqa"),$points,' "'.$last_payment." ".$currency_code.'"').'</p></div>';
			}else {
				$output .= '<div class="alert-message success"><i class="icon-check"></i><p>'.sprintf(esc_html__("Please make a payment to allow to be able to add a question %s.","wpqa"),' "'.$last_payment." ".$currency_code.'"').'</p></div>';
			}
			
			if (isset($coupons) && is_array($coupons) && $free_coupons == "on" && $active_coupons == "on") {
				foreach ($coupons as $coupons_k => $coupons_v) {
					$pay_payments = $last_payments = (int)($price > 0?$price:wpqa_options($payment_option));
					if (isset($coupons_v["coupon_type"]) && $coupons_v["coupon_type"] == "percent") {
			...
			if ($last_payment > 0) {
				$output .= '<div class="payment_area">
					<form method="post" action="?action=process">
						'.($sticky_or_ask == "sticky"?'<input type="hidden" name="question_sticky" value="'.$post_id.'">':'').'
