	...
	function wpqa_comment_question($comment_id) {
		...
		if ($get_post->post_type == "question") {
			...
			if ($get_comment->comment_approved == 1) {
				...
				wpqa_session('<div class="alert-message success"><i class="icon-check"></i><p>'.esc_html__("The answer has added successfully.","wpqa").'</p></div>','wpqa_session');
				wpqa_notifications_add_answer($get_comment,$get_post);
			}else {
				wpqa_session('<div class="alert-message success"><i class="icon-check"></i><p>'.esc_html__("The answer has added successfully, It's under review.","wpqa").'</p></div>','wpqa_session');
				if ($comment_user_id > 0) {
	...
	function wpqa_before_delete_comment($comment_id) {
		...
		if ($remove_best_answer_stats == "on") {
			...
			if ($user_id > 0 && $active_points == "on") {
				...
				add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$point_add_comment,"-","delete_answer","time" => current_time('timestamp')));
			...
			if (isset($best_answer_comment) && isset($comment_id) && $best_answer_comment == "best_answer_comment") {
				if ($user_id > 0 && $active_points == "on") {
					$_points++;
					update_user_meta($user_id,$user_id."_points",$_points);
					add_user_meta($user_id,$user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),$point_best_answer,"-","delete_best_answer","time" => current_time('timestamp')));
					$points_user = get_user_meta($user_id,"points",true);
				...
				if ($point_back_option == "on" && $active_points == "on" && $user_id != $user_author) {
					...
					if ($point_back == "yes" && $user_author > 0) {
						$_points = get_user_meta($user_author,$user_author."_points",true);
						$_points++;
						update_user_meta($user_author,$user_author."_points",$_points);
						add_user_meta($user_author,$user_author."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),($what_point != ""?$what_point:wpqa_options("question_points")),"-","point_removed","time" => current_time('timestamp')));
						$points_user = get_user_meta($user_author,"points",true);
						update_user_meta($user_author,"points",$points_user-($what_point != ""?$what_point:wpqa_options("question_points")));
					}
...
if (!function_exists('wpqa_notifications_add_answer')) :
	function wpqa_notifications_add_answer($comment,$get_post) {
		...
		if ($remember_answer == "on" && $post_author != $comment_user_id) {
			...
			wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),$the_mail,$email_title,$last_message_email);
		}
		...
		if ($question_follow == "on" && isset($following_questions) && is_array($following_questions)) {
			...
			foreach ($following_questions as $user) {
				if ($user_id_question != $user) {
					...
					wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),esc_attr($author_user_email),$email_title,$last_message_email);
				}
			}
		}
		...
		if ($comment_user_id != 0) {
			if ($comment_user_id != $post_author && $active_points == "on") {
				$_points = get_user_meta($comment_user_id,$comment_user_id."_points",true);
				$_points++;
				
				update_user_meta($comment_user_id,$comment_user_id."_points",$_points);
				add_user_meta($comment_user_id,$comment_user_id."_points_".$_points,array(date_i18n('Y/m/d',current_time('timestamp')),date_i18n('g:i a',current_time('timestamp')),(wpqa_options("point_add_comment") != ""?wpqa_options("point_add_comment"):2),"+","answer_question",$post_id,$comment_id,"time" => current_time('timestamp')));
			
				$points_user = get_user_meta($comment_user_id,"points",true);
				update_user_meta($comment_user_id,"points",$points_user+(wpqa_options("point_add_comment") != ""?wpqa_options("point_add_comment"):2));
			}
