		...
		if (is_user_logged_in() && isset($if_user_id->caps["activation"]) && $if_user_id->caps["activation"] == 1) {
			...
			if (isset($_GET['u']) && isset($_GET['activate'])) {
				...
				if (isset($_GET['get_activate']) && $_GET['get_activate'] == "do") {
					...
					wpqa_sendEmail(wpqa_options("email_template"),get_bloginfo('name'),esc_html($user_email),$email_title,$last_message_email);
			...
			if (isset($_GET['u']) && isset($_GET['activate'])) {
				wpqa_session('<div class="alert-message success"><i class="icon-check"></i><p>'.sprintf(wp_kses(__("You are already logged in, You are already active your membership, Go to your <a href='%s'>profile</a>.","wpqa"),array('a' => array('href' => array()))),wpqa_profile_url($user_id)).'</p></div>','wpqa_session');
				wp_safe_redirect(esc_url(home_url('/')));
				die();
			}
		}
