						<?php $confirm_email = (class_exists("WPQA")?wpqa_users_confirm_mail():"");
								...
								<div class="inner-sidebar">
									...
									if ($footer_style == "sidebar") {
										...
										<section class="widget-footer">
											<?php wp_nav_menu(array('container' => '','container_class' => '','menu_id' => 'footer_menu','menu' => $footer_menu,'fallback_cb' => 'discy_nav_fallback'));?>
											<div class="clearfix"></div>
											<p class="credits"><?php echo discy_kses_stip($footer_copyrights)?></p>
										</section>
									<?php }?>
								</div>
							</aside><!-- End sidebar -->
						...
						<nav class="nav_menu float_r<?php echo ($sticky_sidebar == "nav_menu" || $sticky_sidebar == "side_menu_bar"?" fixed_nav_menu":"").($left_menu_style == "style_2"?" nav_menu_2":"").($left_menu_style == "style_3"?" nav_menu_3":"")?>">
							...
							<?php if ($tabs_menu == "on") {
								...
								if (isset($discy_home_tabs) && is_array($discy_home_tabs)) {
									if (isset($first_one)) {?>
										<ul>
											<?php discy_home_tabs($discy_home_tabs,$first_one,"on");?>
										</ul>
									<?php }
								}
							}else {
								...
								wp_nav_menu(array('container' => '','container_class' => 'nav_menu float_r',($left_menu_s != ""?"menu":"theme_location") => ($left_menu_s != "" && $left_menu_s != 0?$left_menu_s:$left_menu),'fallback_cb' => 'discy_nav_fallback'));
							}?>
						</nav><!-- End nav_menu -->
					<?php }
				}?>
...
<?php $go_up_button = discy_options("go_up_button");
if ($go_up_button == "on") {?>
