...
/* Require files */
require_once get_parent_theme_file_path("admin/plugins/class-tgm-plugin-activation.php");
require_once get_parent_theme_file_path("admin/plugins/plugins.php");
require_once get_parent_theme_file_path("admin/includes/fields.php");
...
require_once get_parent_theme_file_path("admin/functions/main_functions.php");
require_once get_parent_theme_file_path("admin/options-class.php");
require_once get_parent_theme_file_path("admin/functions/resizer.php");
require_once get_parent_theme_file_path("admin/functions/widget_functions.php");
require_once get_parent_theme_file_path("admin/functions/nav_menu.php");
require_once get_parent_theme_file_path("admin/functions/register_post.php");
require_once get_parent_theme_file_path("admin/functions/meta_setting.php");
/* Widgets */
...
if (!function_exists('discy_body_classes')) {
	function discy_body_classes($classes) {
		...
		$site_users_only = (class_exists("WPQA")?wpqa_site_users_only():"");
		$wp_page_template = discy_post_meta("_wp_page_template","",false);
		$classes[] = ($wp_page_template == "template-landing.php" || $site_users_only == "yes"?"discy_users_only":"discy_for_all");
...
if (!function_exists('discy_scripts_styles')) {
	function discy_scripts_styles() {
		...
		if (is_rtl()) {
			wp_enqueue_style('discy-main-css', get_template_directory_uri().'/css/rtl.css');
			wp_enqueue_style('discy-main-css', get_template_directory_uri().'/css/rtl-arrows.css');
		}else {
			$site_style = discy_options("site_style");
			if ($site_style == "style_1" || $site_style == "style_2" || $site_style == "style_3" || $site_style == "style_4") {
				wp_enqueue_style('discy-boxed-css', get_template_directory_uri().'/css/boxed-style.css');
			}
			wp_enqueue_style('discy-main-css', get_template_directory_uri().'/css/main.css');
			wp_enqueue_style('discy-responsive', get_template_directory_uri()."/css/responsive.css");
		}
		if (is_category()) {
		...
		if ($site_skin != "default" && $site_skin != "") {
			wp_enqueue_style('discy-skin-'.$site_skin, get_template_directory_uri()."/css/skins/".$site_skin.".css");
		}else {
			wp_enqueue_style('discy-skin-default', get_template_directory_uri()."/css/skins/skins.css");
		}
		...
		if (is_category() || is_tag() || (is_archive() && !is_post_type_archive("question")) || is_tax("question-category") || is_tax("question_tags") || is_post_type_archive("question")) {
			...
			if (is_category() || is_tag() || (is_archive() && !is_post_type_archive("question") && !is_tax("question_tags"))) {
				$background_type = $post_background_type;
			}
			if (is_tax("question-category") || is_tax("question_tags") || is_post_type_archive("question")) {
				$background_type = $question_background_type;
			}
			if (is_tag() || (is_archive() && !is_category() && !is_tax("question-category") && !is_post_type_archive("question") && !is_tax("question_tags"))) {
				...
				if ($background_type == "custom_background" || $background_type == "patterns") {
					...
					$background_full    = $post_full_screen_background;
					$background_color   = $post_background_color;
				}else if (is_tax("question-category") && ($background_type == "default" || $background_type == "")) {
					$background_type    = $question_background_type;
					$background_pattern = $question_background_pattern;
					$custom_background  = $question_custom_background;
					$background_full    = $question_full_screen_background;
		...
		}else if (is_author() || (class_exists('WPQA') && wpqa_is_user_profile())) {
		...
		}else if (is_single() || (class_exists('WPQA') && wpqa_is_search() && wpqa_search_type() == "posts") || is_page()) {
			global $post;
			...
			if (is_singular("post") || (class_exists('WPQA') && wpqa_is_search() && wpqa_search_type() == "posts")) {
			...
			}else if ((is_singular("post") || (class_exists('WPQA') && wpqa_is_search() && wpqa_search_type() == "posts")) && ($background_type == "default" || $background_type == "")) {
				$background_type    = $post_background_type;
				$background_pattern = $post_background_pattern;
				$custom_background  = $post_custom_background;
				$background_full    = $post_full_screen_background;
			...
			if (is_singular("post") || (class_exists('WPQA') && wpqa_is_search() && wpqa_search_type() == "posts") || is_singular("question")) {
		...
		if ((is_category() || is_tag() || (is_archive() && !is_post_type_archive("question")) || is_tax("question-category") || is_tax("question_tags") || is_post_type_archive("question")) && $primary_color_c == "") {
			if ($cat_skin != "default" && $cat_skin != "") {
				if ($cat_skin == "skin") {
					wp_enqueue_style('discy-skin-default', get_template_directory_uri()."/css/skins/skins.css");
				}else if (!empty($cat_skin)) {
					wp_enqueue_style('discy-skin-'.$cat_skin, get_template_directory_uri()."/css/skins/".$cat_skin.".css");
				}
			}else {
		...
		}else if ((is_category() || is_tag() || (is_archive() && !is_post_type_archive("question")) || is_tax("question-category") || is_tax("question_tags") || is_post_type_archive("question")) && $primary_color_c != "") {
			$custom_css .= discy_all_css_color($primary_color_c);
		}else if ((is_author() || (class_exists('WPQA') && wpqa_is_user_profile())) && $primary_color_a == "") {
			if ($discy_skin != "default" && $discy_skin != "") {
				if ($discy_skin == "skin") {
					wp_enqueue_style('discy-skin-default', get_template_directory_uri()."/css/skins/skins.css");
				}else if (!empty($discy_skin)) {
					wp_enqueue_style('discy-skin-'.$discy_skin, get_template_directory_uri()."/css/skins/".$discy_skin.".css");
				}
			}else {
				$primary_color = discy_options("primary_color");
				if ($primary_color != "") {
		...
		}else if ((is_single() || (class_exists('WPQA') && wpqa_is_search() && wpqa_search_type() == "posts") || is_page()) && $primary_color_p == "") {
			if ($discy_skin != "default" && $discy_skin != "") {
				if ($discy_skin == "skin") {
					wp_enqueue_style('discy-skin-default', get_template_directory_uri()."/css/skins/skins.css");
				}else if (!empty($discy_skin)) {
		...
		$site_users_only = (class_exists("WPQA")?wpqa_site_users_only():"");
		$wp_page_template = discy_post_meta("_wp_page_template","",false);
		if ($wp_page_template == "template-landing.php") {
			$register_background = discy_post_meta("register_background");
		}else {
			$register_background = discy_options("register_background");
		}
		if (($site_users_only == "yes" || $wp_page_template == "template-landing.php") && !empty($register_background)) {
			if ($wp_page_template == "template-landing.php") {
		...
		if ((is_user_logged_in() && ($slider_h_logged == "logged" || $slider_h_logged == "both")) || (!is_user_logged_in() && ($slider_h_logged == "unlogged" || $slider_h_logged == "both"))) {
			...
			if ($slider_h == "on" && ($custom_sliders == "on" || (((is_front_page() || is_home()) && $slider_h_home_pages == "home_page") || $slider_h_home_pages == "all_pages" || ($slider_h_home_pages == "custom_pages" && is_page() && isset($slider_h_pages) && is_array($slider_h_pages) && isset($post->ID) && in_array($post->ID,$slider_h_pages))))) {
				if ($custom_sliders == "on") {
		...
		if (isset($main_font["face"]) && $main_font["face"] != "default" && $main_font["face"] != "Default font" && $main_font["face"] != "") {
			...
			$custom_css .= '
			h1,h2,h3,h4,h5,h6,.post-title,.post-title-2,.post-title-3,.widget-posts .user-notifications > div > ul li div h3,.related-widget .user-notifications > div > ul li div h3,.widget-posts .user-notifications > div > ul li div h3 a,.related-widget .user-notifications > div > ul li div h3 a,.accordion .accordion-title,.button-sign-in,.button-sign-up,nav.nav ul li,.menu-tabs > ul > li > a,.nav_menu > ul li a,.nav_menu > div > ul li a,.question-content-text,.widget-title,.user-not-normal .user-area .user-content > .user-inner h4,.about-text,.widget li,.credits,.post-content-text,.button-default,.button-default-2,.button-default-3,a.meta-answer,.load-more a,.post-read-more,.edit-link,.delete-link,.pop-footer,.post-contact form .form-input span,.pagination-wrap .no-comments,.user-follow-profile,.user-area .user-content > .user-inner p,.category-description > p,.social-ul li .user_follow_3 a,.social-ul li a.block_message,.answers-tabs .section-title,.answers-tabs-inner li,.mobile-menu,.mobile-aside li a,.vote_result,.stats-value,.user-stats > ul > li > div > div span,.cat-sections a,.commentlist li.comment .comment-body .comment-text .comment-author a,.point-div span,.ed_button.poll_results,.ed_button.poll_polls,.comment-respond > .section-title,.fileinputs span,.no-results p,.post-author,.related-post .section-title,.navigation-content a,.alert-message,.wpqa-open-click,.question-author-un,.call-action-unlogged p,.panel-image-inner p {
		...
		$custom_css .= '@font-face {
			font-family: "entypo";
			src: url("'.get_template_directory_uri().'/css/entypo/entypo.eot");
			src: url("'.get_template_directory_uri().'/css/entypo/entypo.eot?#iefix") format("embedded-opentype"),
				url("'.get_template_directory_uri().'/css/entypo/entypo.woff") format("woff"),
				url("'.get_template_directory_uri().'/css/entypo/entypo.ttf") format("truetype"),
				url("'.get_template_directory_uri().'/css/entypo/entypo.svg#entypo") format("svg");
			font-weight: normal;
			font-style: normal;
		}';
		...
		wp_enqueue_style('discy-custom-css', get_template_directory_uri().'/css/custom.css');
		...
		wp_enqueue_script("discy-tipsy", get_template_directory_uri()."/js/jquery.tipsy.js",array("jquery"),'1.0.0',true);
		wp_enqueue_script("discy-custom-js", get_template_directory_uri()."/js/custom.js",array("jquery"),'1.0.0',true);
		...
		if ((isset($second_font["face"]) && (($second_font["face"] != "Default font" && $second_font["face"] != "default" && $second_font["face"] != "") || $second_font["face"] == "default" || $second_font["face"] == "Default font" || $second_font["face"] == "") && !in_array($second_font["face"],$safe_fonts)) || (isset($main_font["face"]) && (($main_font["face"] != "Default font" && $main_font["face"] != "default" && $main_font["face"] != "") || $main_font["face"] == "default" || $main_font["face"] != "Default font" || $main_font["face"] == "") && !in_array($main_font["face"],$safe_fonts))) {
			wp_add_inline_script('discy-custom-js',"
				WebFontConfig = {
					google : {
						families: [".(is_rtl()?"'Droid+Arabic+Kufi',":"")."'".(isset($second_font["face"]) && $second_font["face"] != "Default font" && $second_font["face"] != "default" && $second_font["face"] != ""?$second_font["face"]:'Open+Sans').":100,100i,200,200i,300,300i,400,400i,500,500i,600,600i,700,700i,800,800i,900,900i','".(isset($main_font["face"]) && $main_font["face"] != "Default font" && $main_font["face"] != "default" && $main_font["face"] != ""?$main_font["face"]:'Roboto').":100,100i,200,200i,300,300i,400,400i,500,500i,600,600i,700,700i,800,800i,900,900i&amp;subset=cyrillic,cyrillic-ext,greek,greek-ext,latin-ext,vietnamese']
					}
				};
				(function() {
		    ...
		    if (!is_home() && !is_front_page() && (is_single() || is_page())) {
		    	...
		    	if (have_posts()) : while (have_posts()) : the_post();
		    		if (function_exists("wpqa_image_for_share")) {
		        		echo '<meta property="og:image" content="' . wpqa_image_for_share() . '">' . "\n";
		        	}
...
if ( ! function_exists( 'discy_meta' ) ) {
	function discy_meta($date = "",$category = "",$comment = "",$asked = "",$icons = "",$views = "",$post_id = 0) {
		$post_type = get_post_type($post_id > 0?$post_id:"");
		if ( $date == "on" ) {
			$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time>';
			$time_string = sprintf($time_string,esc_attr(get_the_date('c',($post_id > 0?$post_id:""))),esc_html(get_the_time(discy_date_format,($post_id > 0?$post_id:""))));
			...
			echo '<span class="post-date" itemprop="dateCreated" datetime="'.get_the_date("",($post_id > 0?$post_id:"")).'">' . $posted_on . '</span>';
		}
		...
		if ('question' === $post_type) {
			...
			if ( $category == "on" ) {
				echo get_the_term_list(($post_id > 0?$post_id:get_the_ID()),'question-category','<span class="byline"><span class="post-cat">' . esc_html__( 'In: ', 'discy' ),', ','</span></span>');
			}
		}
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) && $comment == "on" ) {
			if ('question' === $post_type) {
				echo (is_singular('question')?'<span itemprop="answerCount" class="number">':'');discy_count_number(comments_popup_link(0,1,'%'));echo (is_singular('question')?'</span>':'');
				if ($icons != "on") {
					echo " <span class='question-span'>";comments_popup_link(esc_html__("Answers","discy"),apply_filters("wpqa_answer_language",esc_html__("Answer","discy")),esc_html__("Answers","discy"));echo "</span>";
				}
			}else {?>
		...
		if ( 'post' === $post_type && $views == "on" && $active_post_stats == "on" ) {
			...
			<span class="post-views">
				<?php esc_html_e('Views: ','discy');
				$post_stats = discy_post_meta($post_meta_stats,"",false);echo ($post_stats != ""?discy_count_number($post_stats):0)?>
			</span>
		<?php }
	}
}?>