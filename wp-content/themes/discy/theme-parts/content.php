<?php
if (!is_array($sort_meta_title_image) || is_single() || $post_style != "style_3") {
...
if (has_post_thumbnail()) {
	...
	if ($featured_image == "on" && isset($wp_page_template) && ($wp_page_template == "template-blog.php" || $wp_page_template == "template-home.php")) {
...
<article id="post-<?php the_ID(); ?>" <?php post_class('article-post article-post-only clearfix'.$featured_style.($post_data->post_content == " post-no-content"?" post--content":"").($post_style == "style_2"?" post-style-2":"").($post_style == "style_3"?" post-style-3".$post_columns:""));?> itemscope="" itemtype="https://schema.org/Article">
	<?php if (isset($sort_meta_title_image) && is_array($sort_meta_title_image)) {
		foreach ($sort_meta_title_image as $sort_meta_title_image_key => $sort_meta_title_image_value) {
			if (isset($sort_meta_title_image_value["value"]) && $sort_meta_title_image_value["value"] == "image") {
				...
				<header class="article-header<?php echo (((isset($wp_page_template) && ($wp_page_template == "template-blog.php" || $wp_page_template == "template-home.php")) || !is_page()) && !is_attachment() && $author_by == "on"?"":" header-no-author").($author_by != "on" && $post_date != "on" && $title_post != "on" && $category_post != "on" && $post_comment != "on" && $post_views != "on"?" header-no-meta":"")?>">
					<?php if ( ((isset($wp_page_template) && ($wp_page_template == "template-blog.php" || $wp_page_template == "template-home.php")) || !is_page()) && !is_attachment() ) {
						if ($post_style == "style_2" || $post_style == "style_3") {
						...
						if ($post_date == "on" || $category_post == "on" || $post_comment == "on" || $post_views == "on") {?>
							<div class="post-meta">
								<?php discy_meta($post_date,$category_post,$post_comment,"","",$post_views)?>
							</div>
						<?php }
					}
					...
					if ($title_post == "on" && !is_attachment()) {
						if ( (is_single() && empty($blog_h)) || (is_page() && empty($blog_h) && (empty($wp_page_template) || ($wp_page_template != "template-blog.php" && $wp_page_template != "template-home.php"))) ) {
							the_title( '<h1 class="'.(isset($title_post_style) && $title_post_style == "style_2"?"post-title-2":"post-title").'">'.(isset($title_post_style) && $title_post_style == "style_2" && isset($title_post_icon) && $title_post_icon != ""?"<i class='".$title_post_icon."'></i>":"").(is_sticky()?"<i class='icon-pencil'></i>":""), '</h1>' );
						}else {
							the_title( '<h2 class="post-title"><a class="post-title" href="' . esc_url( get_permalink() ) . '" rel="bookmark">'.(is_sticky()?"<i class='icon-pencil'></i>":""), '</a></h2>' );
						}
					}
					if ( ((isset($wp_page_template) && ($wp_page_template == "template-blog.php" || $wp_page_template == "template-home.php")) || !is_page()) && !is_attachment() && $author_by == "on" ) {
	...
	<div class="post-wrap-content<?php echo ((is_page() && empty($blog_h) && (empty($wp_page_template) || ($wp_page_template != "template-blog.php" && $wp_page_template != "template-home.php")))?"":" post-content ").(is_attachment()?" post-attachment ":"")?>">
		<div class="<?php echo (isset($wp_page_template) && $wp_page_template == "template-contact.php"?"post-contact":"post-content-text")?>">
			<?php if ( (is_single() && empty($blog_h)) || (is_page() && empty($blog_h) && (empty($wp_page_template) || ($wp_page_template != "template-blog.php" && $wp_page_template != "template-home.php"))) ) {
				if ( is_attachment() ) {
				...
				if (strpos(get_the_content(),'more-link') === false && $post_data->post_content != "") {?>
					<p><?php discy_excerpt($post_excerpt,$excerpt_type);?></p>
				<?php }else {
					the_content("");
				}
			}?>
		</div>
		<?php if (isset($wp_page_template) && $wp_page_template == "template-faqs.php") {
		...
		do_action("wpqa_after_post_tags",$post);?>
	</div>
	...
	<?php if (!is_page_template("template-users.php") && !is_page_template("template-contact.php") && !is_page_template("template-faqs.php") && !is_page_template("template-categories.php") && !is_page_template("template-tags.php")) {?>
		<footer>
			<?php do_action("discy_action_before_edit_post",(isset($blog_h)?$blog_h:""));
			...
			if (is_single() && empty($blog_h)) {
				...
				if (($post_data->post_author != 0 && $post_data->post_author == $user_id) || is_super_admin($user_id)) {
					if (class_exists("WPQA") && ($can_edit_post == "on" || is_super_admin($user_id))) {
						echo '<span class="edit-link"><a href="'.esc_url(wpqa_edit_permalink($post_data->ID,"post")).'">'.esc_html__("Edit","discy").'</a></span>';
					}
					if ($post_delete == "on" || is_super_admin($user_id)) {
						echo '<span class="delete-link post-delete"><a href="'.esc_url(add_query_arg(array("delete" => $post_data->ID,"wpqa_delete_nonce" => wp_create_nonce("wpqa_delete_nonce")),get_permalink($post_data->ID))).'">'.esc_html__("Delete","discy").'</a></span>';
					}
				}
			}
			...
			if ( (strpos(get_the_content(),'more-link') !== false || $read_more == "on") && !is_single() && ((isset($wp_page_template) && ($wp_page_template == "template-blog.php" || $wp_page_template == "template-home.php")) || !is_page()) ) {?>
				<a class="post-read-more" href="<?php echo esc_url(get_permalink())?>" rel="bookmark" title="<?php esc_attr_e('Read','discy')?> <?php the_title()?>"><?php esc_html_e('Read more','discy')?></a>
			<?php }
			if (function_exists("wpqa_share") && empty($blog_h)) {
				...
				$share_google   = (isset($post_share["share_google"]["value"])?$post_share["share_google"]["value"]:"");
				$share_whatsapp = (isset($post_share["share_whatsapp"]["value"])?$post_share["share_whatsapp"]["value"]:"");
				wpqa_share($post_share,$share_facebook,$share_twitter,$share_google,$share_whatsapp);
			}?>
		</footer>
	<?php }
	if ($post_style == "style_2" || $post_style == "style_3") {?>
...
<?php if ( ( (is_single() && empty($blog_h)) || (is_page() && empty($blog_h) && (empty($wp_page_template) || ($wp_page_template != "template-blog.php" && $wp_page_template != "template-home.php"))) ) && !is_attachment() ) :
	...
	foreach ($order_sections as $key_r => $value_r) :
		if ($value_r["value"] == "") :
			...
			<?php elseif (!is_page_template("template-blog.php") && !is_page_template("template-home.php") && !is_page_template("template-users.php") && !is_page_template("template-contact.php") && !is_page_template("template-faqs.php") && !is_page_template("template-categories.php") && !is_page_template("template-tags.php") && $value_r["value"] == "advertising") :
