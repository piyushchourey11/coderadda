...
if ($user_sort == "the_best_answer" || $user_sort == "post_count" || $user_sort == "question_count" || $user_sort == "answers" || $user_sort == "comments") {
	...
	$total_query = $wpdb->get_var( apply_filters( 'found_users_query', 'SELECT FOUND_ROWS()' ) );
...
echo "<div class='user-section user-section-".$user_style.($user_style == "grid" || $user_style == "small" || $user_style == "columns"?" row":"").($user_style != "normal"?" user-not-normal":"")."'>";
	if (isset($query) && !empty($query)) {
		...
		foreach ($query as $user) {
			...
			echo ($user_style == "grid" || $user_style == "small" || $user_style == "columns"?"<div class='col ".($user_style == "columns" && ($discy_sidebar == "sidebar" || $discy_sidebar == "centered" || $discy_sidebar == "menu_left")?"col4":($discy_sidebar == "full"?"col3":"col6"))."'>":"");
			...
			echo ($user_style == "grid" || $user_style == "small" || $user_style == "columns"?"</div>":"");
		}
	}else {
		$no_user = true;
	}
