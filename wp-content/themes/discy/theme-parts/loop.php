...
if (isset($wp_page_template) && (($wp_page_template == "template-home.php" && isset($first_one) && ($first_one == $answers_slug || $first_one == $answers_slug_2)) || $wp_page_template == "template-comments.php")) {
	...
	if (!empty($comments_all)) {
		if (isset($post_pagination) && ($post_pagination == "pagination" || $post_pagination == "standard")) {
			$pagination_args = array(
				...
				'next_text' => ($post_pagination == 'standard'?'<i class="icon-left-open"></i><span>'.esc_html__('Old Answers',"discy").'</span>':'<i class="icon-right-thin"></i>'),
			);
		}
	...
	if (isset($blog_h) && $blog_h == "blog_h") {
	...
	}else if (isset($wp_page_template) && ($wp_page_template == "template-question.php" || $wp_page_template == "template-blog.php" || $wp_page_template == "template-home.php")) {
		include locate_template("includes/templates.php");
		
		if ($wp_page_template == "template-home.php" && isset($first_one)) {
			$get_tax = get_term_by('slug',$first_one,"question-category");
		}

		if (isset($wp_page_template) && $wp_page_template == "template-question.php" && isset($orderby_post) && ($orderby_post == "popular" || $orderby_post == "most_visited" || $orderby_post == "most_voted")) {
			$active_sticky = false;
			$array_data    = array_merge($orderby_array,$post__not_in,$cats_post,$specific_date_array,array("post_type" => $post_type,"ignore_sticky_posts" => 1,"paged" => $paged,"posts_per_page" => $post_number));
		}else if ($wp_page_template == "template-home.php" && isset($first_one) && $first_one == "all") {
			$active_sticky = true;
			$custom_args   = array("post_type" => "question");
			$show_sticky   = true;
			$array_data    = array_merge($orderby_array,$post__not_in,array("post_type" => $post_type,"ignore_sticky_posts" => 1,"paged" => $paged,"posts_per_page" => $post_number));
		}else if ($wp_page_template == "template-home.php" && isset($first_one) && is_string($first_one) && isset($get_tax->term_id) && $get_tax->term_id > 0) {
			$active_sticky = true;
			$custom_args   = array("post_type" => "question","tax_query" => array(array("taxonomy" => "question-category","field" => "id","terms" => $get_tax->term_id)));
			$show_sticky   = true;
			$array_data    = array_merge($orderby_array,$post__not_in,$cats_post,array("post_type" => $post_type,"ignore_sticky_posts" => 1,"paged" => $paged,"posts_per_page" => $post_number,"tax_query" => array(array("taxonomy" => "question-category","field" => "slug","terms" => $first_one))));
		}else if (($wp_page_template == "template-question.php" && isset($orderby_post) && $orderby_post == "sticky") || ($wp_page_template == "template-home.php" && isset($first_one) && ($first_one == $question_sticky_slug || $first_one == $question_sticky_slug_2))) {
			$active_sticky = true;
			$sticky_only   = true;
			$custom_args   = array_merge($specific_date_array,array("post_type" => "question"));
			$show_sticky   = true;
		}else if (($wp_page_template == "template-question.php" && isset($orderby_post) && $orderby_post == "polls") || ($wp_page_template == "template-home.php" && isset($first_one) && ($first_one == $question_polls_slug || $first_one == $question_polls_slug_2))) {
			$active_sticky = true;
			$custom_args   = array("post_type" => "question");
			$show_sticky   = true;
			...
			$array_data    = array_merge($post__not_in,$specific_date_array,$poll_array,array("post_type" => "question","paged" => $paged,"posts_per_page" => $post_number));
		}else {
		...
		if ((($wp_page_template == "template-question.php" || $wp_page_template == "template-home.php")) && ($orderby_post == "no_answer" || ($question_bump == "on" && $active_points == "on" && $orderby_post == "question_bump"))) {
		...
		$array_data = array_merge($post__not_in,$poll_array,array("post_type" => "question","paged" => $paged));
	}else if (is_tax("question-category")) {
		$question_numbers = array();
		if (isset($post_number) && $post_number > 0) {
			$question_numbers = array("posts_per_page" => $post_number);
		}
		$array_data = array_merge($question_numbers,$post__not_in,array("ignore_sticky_posts" => 1,"post_type" => "question","paged" => $paged,"tax_query" => array(array("taxonomy" => "question-category","field" => "id","terms" => $category_id)),"meta_query" => array($question_meta_query)));
	}else if (is_tax("question_tags")) {
		$array_data = array_merge($post__not_in,array("ignore_sticky_posts" => 1,"post_type" => "question","paged" => $paged,"tax_query" => array(array("taxonomy" => "question_tags","field" => "id","terms" => $category_id,"operator" => "IN")),"meta_query" => array($question_meta_query)));
		...
		if (is_array($_favorites) && !empty($_favorites) && count($_favorites) >= 1) {
		...
		if (is_array($following_questions_user) && !empty($following_questions_user) && count($following_questions_user) >= 1) {
		...
		if (is_array($following_me) && count($following_me) >= 1) {
		...
		<?php if ($last_one == "favorites" && empty($_favorites)) {
			...
			if (isset($custom_args) || (isset($wpqa_query) && $wpqa_query->have_posts()) || (have_posts() && empty($array_data))) :
				...
				if (!isset($sticky_only)) :
					if (isset($wpqa_query) && $wpqa_query->have_posts() !== null) {
