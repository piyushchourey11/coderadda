...
if (is_admin()) {
	...
	function discy_screen_belongs_to_theme_options() {
		...
		if ( ( ($pagenow == 'themes.php' || $pagenow == 'admin.php') && array_key_exists( 'page', $_GET ) && $_GET['page'] == 'options' ) || ( ($pagenow == 'themes.php' || $pagenow == 'admin.php') && array_key_exists( 'page', $_GET ) && $_GET['page'] == 'r_questions' ) || ( ($pagenow == 'themes.php' || $pagenow == 'admin.php') && array_key_exists( 'page', $_GET ) && $_GET['page'] == 'r_answers' ) || ( ($pagenow == 'themes.php' || $pagenow == 'admin.php') && array_key_exists( 'page', $_GET ) && $_GET['page'] == 'wpqa_payments' ) || in_array( $pagenow, $white_label ) || $show_admin_scripts == true ) {
			return true;
		}
		return false;
	}
...
/* discy_counter_googleplus */
function discy_counter_googleplus ($page_id, $return = 'count') {
	$count = get_transient('discy_googleplus_followers');
	$link = get_transient('discy_googleplus_page_url');
	if ($return == 'link') {
		if ($link !== false) return $link;
	}else {
		if ($count !== false) return $count;
	...
	if (!is_wp_error($data)) {
		...
		$link = $json['url'];
		set_transient('discy_googleplus_followers', $count, 60*60*24);
		set_transient('discy_googleplus_page_url', $link, 60*60*24);
	}
	if ($return == 'link') {
		return $link;
	}else {
            ...
            <div class="comment-text">
                ...
                <div class="author clearfix">
                	...
                	<div class="comment-meta">
                    	<div class="comment-author">
                    		...
                    		if ($wpqa_activate_comment_author == true) {
	                    		...
	                    		if ($get_author_posts_url != "" && $get_author_posts_url != "discy_No_site") {?>
	                    			<a<?php echo (isset($its_question) && $its_question == 'question'?' itemprop="url"':'')?> href="<?php echo esc_url($get_author_posts_url)?>">
	                    		<?php }
	                    			if (isset($its_question) && $its_question == 'question') {
                    	...
                    	<a href="<?php echo get_comment_link($comment_id); ?>" class="comment-date" itemprop="dateCreated" datetime="<?php echo get_comment_date("",$comment_id)?>">
                    		<?php $date = mysql2date(discy_time_format,$comment->comment_date,true);
                    		if (isset($its_question) && $its_question == "question") {
                    			echo ($comment->comment_parent > 0?esc_html__("Replied to","discy"):esc_html__("Added an","discy"))." ";
                    			printf(esc_html__('answer on %1$s at %2$s','discy'),get_comment_date("",$comment_id),$date);
                    		}else {
                    			echo ($comment->comment_parent > 0?esc_html__("Replied to","discy"):esc_html__("Added a","discy"))." ";
                    			printf(esc_html__('comment on %1$s at %2$s','discy'),get_comment_date("",$comment_id),$date);
                    		}?>
                    	</a>
                ...
                <div class="text">
                		...
                		if ($active_vote == "on" && $active_vote_answer == "on") {
                			if ($owner == false) {?>
	                			<ul class="question-vote answer-vote<?php echo ($show_dislike_answers != "on"?" answer-vote-dislike":"")?>">
	                				...
	                				<li class="vote_result" itemprop="upvoteCount"><?php echo ($comment_vote != ""?discy_count_number($comment_vote):0)?></li>
                	...
                	<ul class="comment-reply comment-reply-main">
                	    ...
                	    if (function_exists("wpqa_share") && ($share_facebook == "share_facebook" || $share_twitter == "share_twitter" || $share_google == "share_google" || $share_whatsapp == "share_whatsapp")) {?>
	                		...
	                		<li class="question-list-details comment-list-details">
								...
								<ul>
			                	    ...
			                	    if (class_exists("WPQA") && (($can_delete_comment == "on" && $comment->user_id == $user_get_current_user_id && $comment->user_id > 0 && $user_get_current_user_id > 0) || current_user_can('edit_comment',$comment_id) || is_super_admin($user_get_current_user_id))) {
			                	    	echo "<li><a class='delete-comment".(isset($its_question) && $its_question == "question"?' delete-answer':'')."' href='".esc_url(add_query_arg(array('delete_comment' => $comment_id,"wpqa_delete_nonce" => wp_create_nonce("wpqa_delete_nonce")),get_permalink($comment->comment_post_ID)))."'><i class='icon-trash'></i>".esc_html__("Delete","discy")."</a></li>";
			                	    }
			            	    	if (isset($its_question) && $its_question == "question" && $active_reports == "on" && ((is_user_logged_in() && $comment->user_id != $user_get_current_user_id && $comment->user_id != 0 && $user_get_current_user_id != 0) || (!is_user_logged_in() && $active_logged_reports != "on"))) {
			            	    		if ($owner == false) {?>
			                	    		<li><a class="report_c" href="<?php echo esc_attr($comment_id)?>"><i class="icon-attention"></i><?php esc_html_e("Report","discy")?></a></li>
			                	    	<?php }
			                	    }?>
			                	</ul>
			                </li>
...
if (!function_exists('discy_pre_get_posts')) {
	function discy_pre_get_posts($query) {
		...
		if (is_category()) {
			$tax_id         = get_term_by('slug',esc_attr(get_query_var('category_name')),'category');
			$category_id    = $tax_id->term_id;
			$category_style = discy_term_meta("post_style",$category_id);
			$category_style = ($category_style != ""?$category_style:"default");
			if ($category_style != "default") {
				$post_number = discy_term_meta("post_number",$category_id);
			}
		}else if (is_tax("question-category")) {
			$tax_id         = get_term_by('slug',esc_attr(get_query_var('question-category')),'question-category');
			$category_id    = $tax_id->term_id;
			$category_style = discy_term_meta("question_columns",$category_id);
			$category_style = ($category_style != ""?$category_style:"default");
			if ($category_style != "default") {
	...
	function discy_sidebars($return = 'sidebar_dir') {
		...
		if (is_author() || (class_exists('WPQA') && wpqa_is_user_profile())) {
			...
			if ($sidebar_post == "" || $sidebar_post == "default") {
				...
				if ((is_singular("post") || (class_exists('WPQA') && wpqa_is_search() && wpqa_search_type() == "posts")) && $cat_sidebar_layout != "default" && $cat_sidebar_layout != "") {
	...
	if (($adv_type == "display_code" && $adv_code != "" && $adv_code != "empty") || ($adv_type == "custom_image" && $adv_img != "")) {
		$output .= '<'.($li == "li"?"li":"div").' class="discy-ad'.($class != ""?" ".$class:"").'">
			...
			if ($question_columns == "style_2") {
				$output .= '<div class="question-2-columns-border"></div>';
			}
...
/* Home tab setting */
function discy_home_setting ($discy_home_tabs) {
...
/* Home tabs */
function discy_home_tabs ($discy_home_tabs,$first_one,$tabs_menu = "") {
	$question_bump = discy_options("question_bump");
	$active_points = discy_options("active_points");
	include locate_template("includes/slugs.php");
	foreach ($discy_home_tabs as $key => $value) {
		if ((isset($discy_home_tabs[$key]["sort"]) && isset($discy_home_tabs[$key]["value"]) && $discy_home_tabs[$key]["value"] != "" && $discy_home_tabs[$key]["value"] != "0") || (isset($discy_home_tabs[$key]["value"]) && isset($discy_home_tabs[$key]["cat"]))) {
			...
			<li<?php echo (isset($first_one) && $first_one == $last_url?" class='active-tab'":"")?>>
				<a href="<?php echo esc_url(add_query_arg(array("show" => esc_attr($last_url))))?>">
					<?php if ($tabs_menu == "on" && isset($class) && $class != "") {?>
