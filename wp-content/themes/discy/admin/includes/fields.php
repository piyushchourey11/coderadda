...
function discy_options_fields($settings = array(),$option_name = "",$page = "options",$post_term,$options_arrgs = array()) {
	...
	foreach ( $options as $value ) {
		...
		if ($value['type'] != "heading") {
			...
			if ($value['type'] != "heading-2" && $value['type'] != "content" && $value['type'] != "info") {
				// Set default value to $val
				if ( isset( $value['std'] ) ) {
					$val = $value['std'];
				}
				
				$field_id = esc_attr(($page == "widgets"?$post_term->get_field_id( $value['id'] ):(isset($value_name_id)?$value_name_id:$value['id'])));
			...
			if ($value['type'] != 'heading' && $value['type'] != "heading-2" && $value['type'] != 'info' && $value['type'] != "content" && array_key_exists( 'id', $value )) {
			...
			if ($value['type'] != "heading-2" && $value['type'] != "content" && $value['type'] != "info" && $value['type'] != 'hidden') {
				$output .= '<div data-type="'.$value['type'].'" data-id="'.esc_attr( $value['id'] ).'"'.( $condition ).( $operator ).' id="'.esc_attr( $id ).'" class="'.esc_attr( $class ).'"'.(isset($value['margin']) && $value['margin'] != ""?" style='margin:".$value['margin']."'":"").'>'."\n";
				if ( isset( $value['name'] ) ) {
		...
		// Text input
		case 'text':
			$output .= '<input id="'.$field_id.'" class="discy-input discy-form-control" name="'.$field_name.'" type="text" value="' . esc_attr( $val ) . '">';
			break;
		...
		case 'questions_categories':
			...
			<div class="styled-select">'.
				wp_dropdown_categories(array(
				    ...
				    'name'            => "",
				    'show_option_all' => __('Show All Categories','discy'),
				)).'
			</div>
			<div class="clear"></div>
			<div class="add-item add-item-2 add-item-6 add-item-7" data-toadd="'.(isset($value['toadd']) && $value['toadd'] != ""?$value['toadd']:"").'" data-addto="'.(isset($field_id) && $field_id != ""?$field_id:"").'" data-id="'.(isset($field_id) && $field_id != ""?$field_id:"").'_categories" data-name="'.(isset($field_id) && $field_id != ""?$field_id:"").'">Add category</div>
			<div class="clear"></div>
					...
					$output .= '<ul id="'.(isset($field_id) && $field_id != ""?$field_id:"").'-ul" class="sort-sections sort-sections-ul">';
						...
						if(isset($val) && is_array($val)) {
							foreach ($val as $key_a => $value_a) {
								...
								$i++;
								$output .= '<li class="li class="categories"" id="'.(isset($field_id) && $field_id != ""?$field_id:"").'_categories_'.$value_a.'"><div class="widget-head">
									<span>'.($value_a == 0?__('Show All Categories','discy'):$term_cat->name).'</span></div><input name="'.(isset($field_id) && $field_id != ""?$field_id:"").'[cat-'.$value_a.']" value="'.$value_a.'" type="hidden">
									<div>
		...
		case 'elements':
					...
					<li>';
						...
						$output .= '<div class="widget-content">';
							foreach ($value['options'] as $key_e => $value_e) {
								...
								if ($value_e["type"] != "hidden_id" && $value_e["type"] != "heading-2") {
									...
									if ($value_e["type"] == "images") {
										...
										$output .= '<textarea data-attr="'.$value_e["id"].'" class="discy-input" rows="'.$rows.'" id="' . esc_attr( $value_e['id'] ) . '">'.(isset($value_s[$value_e['id']])?stripslashes($value_s[$value_e['id']]):"").'</textarea>';
									}else if ($value_e["type"] == "heading-2") {
			...
			<input class="add_element'.(isset($value['not_theme']) && $value['not_theme'] == "not"?" no_discy_theme":"").(isset($value['addto']) && $value['addto'] != ""?" add_element_to":"").'" type="button" value="'.(isset($value['button']) && $value['button'] != ""?$value['button']:esc_html__("+ Add new element","discy")).'"'.(isset($field_id) && $field_id != ""?" data-id='".$field_id."'":"").(isset($value['title']) && $value['title'] != ""?" data-title='".$value['title']."'":"").'>
			<div class="clear"></div>
			<ul class="sort-sections sort-sections-with sort-sections-ul" id="'.(isset($field_id) && $field_id != ""?$field_id:"").'">';
				$i = 0;
						...
						$output .= '<li id="elements_'.$field_id.'_'.$i.'">';
							...
							$output .= '<div class="widget-content">';
								foreach ($value['options'] as $key_l => $value_l) {
									...
									if ($value_l["type"] != "hidden_id" && $value_l["type"] != "heading-2") {
										...
										if ($value_l["type"] == "images") {
											...
											$output .= "<div class='controls controls-upload'>".discy_options_uploader($field_id,(isset($value_s[$value_l['id']])?$value_s[$value_l['id']]:""),null,$field_name.'['.$i.']['.$value_l['id'].']')."</div>";
											...
											'</div>';
										}else if ($value_l["type"] == "heading-2") {
											...
											$output .= '<input'.($value_l["type"] == "color"?" class='discy-color'":"").($value_l["type"] == "date"?" class='discy-datepicker'":"").' name="'.$field_name.'['.$i.']['.$value_l['id'].']" type="'.($value_l["type"] == "hidden_id"?"hidden":"text").'" value="'.stripslashes(isset($value_s[$value_l['id']])?$value_s[$value_l['id']]:"").'">';
										}
									if ($value_l["type"] != "hidden_id" && $value_l["type"] != "heading-2") {
										$output .= '<div class="clear"></div></div>';
									}
		...
		case 'roles':
			...
			if (isset($role["activation"])) {
				unset($role["activation"]);
			}
			$output .= '
			<input id="role_name" type="text" name="role_name" value="">
			<input id="role_add" data-id="'.$field_id.'" type="button" class="add_element not_add_element" value="'.esc_html__("Add new group","discy").'">
			<div class="clear"></div>
			<ul id="roles_list" class="not-sort sort-sections sort-sections-ul roles_list">';
				if($role) {
					foreach ($role as $key_rol => $value_rol) {$k++;
						...
						$output .= '<li><div class="widget-head">'.(isset($value_rol["group"])?esc_attr($value_rol["group"]):esc_attr($value_rol["name"])).(isset($value_rol["new"]) && $value_rol["new"] == "new"?'<a class="del-roles-item del-builder-item del-sidebar-item"><span class="dashicons dashicons-trash"></span></a>':'').'</div>
							<div class="widget-content">
								<div class="widget-content-div">';
									...
									$output .= '<input type="hidden" class="group_role_name" name="'.$field_name.'['.$last_id.'][id]" value="'.(isset($key_rol) && is_string($key_rol) && isset($value_rol["id"]) && is_array($wp_roles->roles[$key_rol])?esc_attr($value_rol["id"]):$last_id).'">
									<div class="clearfix"></div>
									
									<div class="section section-checkbox">
										<div class="option">
											<div class="controls">
												<label class="switch" for="'.$field_id.'['.$last_id.'][ask_question]">
													<input id="'.$field_id.'['.$last_id.'][ask_question]" value="on" class="checkbox discy-input" type="checkbox" name="'.$field_name.'['.$last_id.'][ask_question]" '. (isset($value_rol["ask_question"])?checked( $value_rol["ask_question"], "on", false):"") .'>
													<label for="'.$field_id.'['.$last_id.'][ask_question]" data-on="'.esc_attr__("ON","discy").'" data-off="'.esc_attr__("OFF","discy").'"></label>
												</label>
												<label class="explain explain-checkbox" for="'.$field_id.'['.$last_id.'][ask_question]">'.esc_html__("Select ON to add a question.","discy").'</label>
											</div>
										</div>
									</div>
									...
									<div class="section section-checkbox">
										<div class="option">
											<div class="controls">
												<label class="switch" for="'.$field_id.'['.$last_id.'][show_question]">
													<input id="'.$field_id.'['.$last_id.'][show_question]" value="on" class="checkbox discy-input" type="checkbox" name="'.$field_name.'['.$last_id.'][show_question]" '. (isset($value_rol["show_question"])?checked( $value_rol["show_question"], "on", false):"") .'>
													<label for="'.$field_id.'['.$last_id.'][show_question]" data-on="'.esc_attr__("ON","discy").'" data-off="'.esc_attr__("OFF","discy").'"></label>
												</label>
												<label class="explain explain-checkbox" for="'.$field_id.'['.$last_id.'][show_question]">'.esc_html__("Select ON to show questions.","discy").'</label>
											</div>
										</div>
									</div>
									...
									<div class="section section-checkbox">
										<div class="option">
											<div class="controls">
												<label class="switch" for="'.$field_id.'['.$last_id.'][add_answer]">
													<input id="'.$field_id.'['.$last_id.'][add_answer]" value="on" class="checkbox discy-input" type="checkbox" name="'.$field_name.'['.$last_id.'][add_answer]" '. (isset($value_rol["add_answer"])?checked( $value_rol["add_answer"], "on", false):"") .'>
													<label for="'.$field_id.'['.$last_id.'][add_answer]" data-on="'.esc_attr__("ON","discy").'" data-off="'.esc_attr__("OFF","discy").'"></label>
												</label>
												<label class="explain explain-checkbox" for="'.$field_id.'['.$last_id.'][add_answer]">'.esc_html__("Select ON to add an answer.","discy").'</label>
											</div>
										</div>
									</div>
									...
									<div class="section section-checkbox">
										<div class="option">
											<div class="controls">
												<label class="switch" for="'.$field_id.'['.$last_id.'][show_answer]">
													<input id="'.$field_id.'['.$last_id.'][show_answer]" value="on" class="checkbox discy-input" type="checkbox" name="'.$field_name.'['.$last_id.'][show_answer]" '. (isset($value_rol["show_answer"])?checked( $value_rol["show_answer"], "on", false):"") .'>
													<label for="'.$field_id.'['.$last_id.'][show_answer]" data-on="'.esc_attr__("ON","discy").'" data-off="'.esc_attr__("OFF","discy").'"></label>
												</label>
												<label class="explain explain-checkbox" for="'.$field_id.'['.$last_id.'][show_answer]">'.esc_html__("Select ON to show answers.","discy").'</label>
											</div>
										</div>
									</div>
									...
									<div class="section section-checkbox">
										<div class="option">
											<div class="controls">
												<label class="switch" for="'.$field_id.'['.$last_id.'][add_post]">
													<input id="'.$field_id.'['.$last_id.'][add_post]" value="on" class="checkbox discy-input" type="checkbox" name="'.$field_name.'['.$last_id.'][add_post]" '. (isset($value_rol["add_post"])?checked( $value_rol["add_post"], "on", false):"") .'>
													<label for="'.$field_id.'['.$last_id.'][add_post]" data-on="'.esc_attr__("ON","discy").'" data-off="'.esc_attr__("OFF","discy").'"></label>
												</label>
												<label class="explain explain-checkbox" for="'.$field_id.'['.$last_id.'][add_post]">'.esc_html__("Select ON to add a post.","discy").'</label>
											</div>
										</div>
									</div>
									...
									<div class="section section-checkbox">
										<div class="option">
											<div class="controls">
												<label class="switch" for="'.$field_id.'['.$last_id.'][send_message]">
													<input id="'.$field_id.'['.$last_id.'][send_message]" value="on" class="checkbox discy-input" type="checkbox" name="'.$field_name.'['.$last_id.'][send_message]" '. (isset($value_rol["send_message"])?checked( $value_rol["send_message"], "on", false):"") .'>
													<label for="'.$field_id.'['.$last_id.'][send_message]" data-on="'.esc_attr__("ON","discy").'" data-off="'.esc_attr__("OFF","discy").'"></label>
												</label>
												<label class="explain explain-checkbox" for="'.$field_id.'['.$last_id.'][send_message]">'.esc_html__("Select ON to send message.","discy").'</label>
											</div>
										</div>
									</div>
									...
									<div class="section section-checkbox">
										<div class="option">
											<div class="controls">
												<label class="switch" for="'.$field_id.'['.$last_id.'][upload_files]">
													<input id="'.$field_id.'['.$last_id.'][upload_files]" value="on" class="checkbox discy-input" type="checkbox" name="'.$field_name.'['.$last_id.'][upload_files]" '. (isset($value_rol["upload_files"])?checked( $value_rol["upload_files"], "on", false):"") .'>
													<label for="'.$field_id.'['.$last_id.'][upload_files]" data-on="'.esc_attr__("ON","discy").'" data-off="'.esc_attr__("OFF","discy").'"></label>
												</label>
												<label class="explain explain-checkbox" for="'.$field_id.'['.$last_id.'][upload_files]">'.esc_html__("Select ON to can upload files.","discy").'</label>
											</div>
										</div>
									</div>
								</div>
		...
		case 'coupons':
			...
			<input id="coupon_date" name="coupon_date" class="discy-datepicker coupon_date" type="text" value="">
					...
					foreach ($coupons as $coupons_k => $coupons_v) {
						$output .= '<li>
							...
							<div class="widget-content">
								...
								<h4 class="heading">Expiry date</h4>
								<input class="discy-datepicker coupon_date" type="text" name="coupons['.esc_html($coupons_k).'][coupon_date]" value="'.esc_html($coupons_v["coupon_date"]).'">
			...
			foreach ($value_option as $key => $option) {
				...
				if (isset($value['sort']) && $value['sort'] == "yes") {
					...
					if (isset($val[$key]["cat"]) && $val[$key]["cat"] == "yes") {
						$label = ($val[$option]["value"] > 0?get_term($val[$option]["value"],"question-category")->name:esc_html__("Show All Categories","discy"));
					}else {
				...
				$output .= '<li'.(isset($value['sort']) && $value['sort'] == "yes" && isset($val[$key]["cat"]) && $val[$key]["cat"] == "yes"?" class='categories' id='".$value['id']."_categories_".$val[$key]["value"]."'":'').'>';
					if (isset($value['sort']) && $value['sort'] == "yes") {
						...
						if (isset($val[$key]["cat"]) && $val[$key]["cat"] == "yes") {
							$output .= '<input name="'.$value['id'].'[cat-'.$val[$key]["value"].'][cat]" value="yes" type="hidden"><input name="'.$value['id'].'[cat-'.$val[$key]["value"].'][value]" value="'.$val[$key]["value"].'" type="hidden">';
						}else {
		...
		// Info
		case 'info':
			$output .= '<div data-type="'.$value['type'].'"'.(isset($value['id'])?' data-id="'.esc_attr( $value['id'] ).'"':"").( $condition ).( $operator ).(isset($id)?' id="'.esc_attr( $id ).'"':'').' class="'.esc_attr( $class ).'">'."\n";
			if ( isset($value['name']) ) {
