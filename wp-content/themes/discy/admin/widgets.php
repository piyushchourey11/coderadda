...
function discy_admin_widgets() {
	...
	$options['widget_counter'] = array(
		...
		array(
			'name' => esc_html__('Google Plus Page ID/Name','discy'),
			'id'   => 'gplus',
			'type' => 'text',
			'std'  => '+envato'
		),
	...
	$options['video-widget'] = array(
		...
		array(
			...
			'id'      => 'video_type',
			'options' => array("youtube" => esc_html__("Youtube","discy"),"vimeo" => esc_html__("Vimeo","discy"),"daily" => esc_html__("Dialymotion","discy"),"facebook" => esc_html__("Facebook video","discy"),"embed" => esc_html__("Embed Code","discy")),
			'std'     => 'draft',
			'type'    => 'select'
		),
		array(
	...
	$options['comments-post-widget'] = array(
		...
		array(
			...
			'id'      => 'post_or_question',
			'options' => array("post" => esc_html__("Post","discy"),"question" => esc_html__("Question","discy")),
			'std'     => 'post',
			'type'    => 'radio'
		),
		array(
...
function discy_save_widget( $instance, $new_instance, $old_instance, $widget ) {
	...
	if (array_key_exists($widget->id_base,$options)) {
		...
		if (array_key_exists("twitter-widget",$options) && $widget->id_base == "twitter-widget") {
			...
			delete_transient('discy_facebook_page_url');
			delete_transient('discy_twitter_followers');
			delete_transient('discy_vimeo_followers');
			delete_transient('discy_vimeo_page_url');
			delete_transient('discy_googleplus_followers');
			delete_transient('discy_googleplus_page_url');
